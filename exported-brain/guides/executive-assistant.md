...
on email_batch(emails):
    for email in emails:
        ...
        sender_page = modusbrain search "{email.sender_name}"
        if sender_page:
            context = modusbrain get <sender_slug>
...
on upcoming_meeting(meeting):
    ...
    for attendee in meeting.attendees:
        ...
        results = modusbrain search "{attendee.name}"
        if results:
            page = modusbrain get <attendee_slug>
            briefing[attendee] = {
                ...
                "relevant_deals": modusbrain get_links <attendee_slug>,
            }
        else:
...
on inbox_cleared():
    for email in processed_emails:
        if email.contained_new_information:
            ...
            modusbrain add_timeline_entry <sender_slug> \
...
2. **Unknown senders with no brain page are almost always noise.** If `modusbrain search` returns nothing for a sender, they're probably not important. Classify as low priority unless the email content signals otherwise.
