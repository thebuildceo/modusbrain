...
import type { BrainEngine } from 'modusbrain/engine';
...
The v0.11.0 migration orchestrator (run by `modusbrain apply-migrations`)
...
```json
{
  ...
  "recommendation": "Add a handler registration for `ea-inbox-sweep` in your host worker bootstrap per docs/guides/plugin-handlers.md. Once registered, re-run `modusbrain apply-migrations` to auto-rewrite this entry.",
  "status": "pending"
}
```
...
4. Re-run `modusbrain apply-migrations --yes`. The orchestrator now
   ...
   cron entry to use `modusbrain jobs submit`. The JSONL row is marked
   `status: "complete"`.
