...
3. If prompted to configure the consent screen:
   ...
   - App name: 'ModusBrain' (anything works)
...
```bash
mkdir -p ~/.modusbrain/integrations/credential-gateway
echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","event":"setup_complete","source_version":"0.7.0","status":"ok","details":{"type":"CLAWVISOR_OR_GOOGLE"}}' >> ~/.modusbrain/integrations/credential-gateway/heartbeat.jsonl
```
...
2. **Google OAuth tokens expire.** Access tokens last ~1 hour. The refresh token
   is long-lived but can be revoked. Store both in `~/.modusbrain/google-tokens.json`
...
2. **Google OAuth:** Tokens exist at `~/.modusbrain/google-tokens.json`.
...
*Part of the [ModusBrain Skillpack](../docs/MODUSBRAIN_SKILLPACK.md). See also: [Email-to-Brain](email-to-brain.md), [Calendar-to-Brain](calendar-to-brain.md)*
