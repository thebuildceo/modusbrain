 ...
 * host's checked-in `skills/RESOLVER.md`, modusbrain publishes an aggregated
 * `~/.modusbrain/mounts-cache/RESOLVER.md` + `manifest.json` whenever mounts
...
function getMountsCacheDir(): string {
  return modusbrainPath('mounts-cache');
}
...
export function renderResolverMarkdown(composed: ComposedResolver): string {
  const lines: string[] = [];
  lines.push('# ModusBrain Skill Resolver (aggregated)');
  lines.push('');
  lines.push('Auto-generated by `modusbrain mounts add|remove|sync`. Do not edit by hand.');
...
export function renderManifestJson(composed: ComposedManifest): string {
  return JSON.stringify(
    {
      generated_by: 'modusbrain mounts',
...
/**
 * Write the aggregated cache to ~/.modusbrain/mounts-cache/. Safe to call
...
export function writeMountsCache(
  ...
  // Unique tmp names per call so concurrent `modusbrain mounts add|remove`
...
/** Remove the aggregated cache dir. Called by `modusbrain mounts remove` and tests. */
