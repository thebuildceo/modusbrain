 ...
 *
 *   3. Audit dir resolution. Honors `MODUSBRAIN_AUDIT_DIR` env override
 *      ahead of the default `~/.modusbrain/audit/`. Trim whitespace; an
...
/**
 * Resolve the audit dir. Honors `MODUSBRAIN_AUDIT_DIR` for container /
...
export interface AuditWriterOpts {
   ...
   * (`'shell-audit'` vs `'modusbrain'` vs `'supervisor-audit'`) so the
...
export interface AuditWriter<T extends { ts: string }> {
  ...
  /** Resolve the audit directory (honors MODUSBRAIN_AUDIT_DIR override). */
  resolveDir(): string;
}
