/**
 * Qrels file format for `modusbrain eval gate --qrels`.
 *
...
export function parseQrelsFile(content: string): QrelsFile {
  ...
  if (file.schema_version !== QRELS_FILE_SCHEMA_VERSION) {
    throw new QrelsParseError(
      `Unsupported schema_version ${file.schema_version} (this modusbrain build expects ${QRELS_FILE_SCHEMA_VERSION})`,
    );
  }
