...
export function validateSchemaPackTrustGate(
  ...
  if (ctx.remote !== false) {
    throw new SchemaPackTrustGateError(
      ...
      'Pass via modusbrain.yml `schema:` section, ~/.modusbrain/config.json `schema_pack`, ' +
      'MODUSBRAIN_SCHEMA_PACK env var, or `modusbrain config set schema_pack <name>`. ' +
    ...
    if (scope.sourceIds.length === 1) {
      ...
      for (const sid of scope.sourceIds) {
        const res = resolveActivePackName({
          ...
          envVar: process.env.MODUSBRAIN_SCHEMA_PACK?.trim() || undefined,
          sourceId: sid,
