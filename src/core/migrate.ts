...
export const MIGRATIONS: Migration[] = [
  ...
  {
    version: 123,
    name: 'operational_skills_layer',
    // ModusBrain / Company Brain extension — versioned operational skills,
    // confidence gating, conflict queue, action audit, correction feedback.
    idempotent: true,
    sql: `
      ...
      CREATE TABLE IF NOT EXISTS operational_skill_versions (
        id SERIAL PRIMARY KEY,
        skill_id INTEGER NOT NULL REFERENCES operational_skills(id) ON DELETE CASCADE,
        version INTEGER NOT NULL,
        slug TEXT NOT NULL,
        title TEXT NOT NULL,
        status TEXT NOT NULL DEFAULT 'draft'
          CHECK (status IN ('draft', 'approved', 'superseded', 'blocked')),
        risk_tier TEXT NOT NULL DEFAULT 'low_stakes'
          CHECK (risk_tier IN ('informational', 'low_stakes', 'high_stakes')),
        conflict_policy TEXT NOT NULL DEFAULT 'serve_last_approved'
          CHECK (conflict_policy IN ('serve_last_approved', 'block')),
        prose_judgment TEXT NOT NULL DEFAULT '',
        structured_policy JSONB NOT NULL DEFAULT '{}',
      ...
      ALTER TABLE operational_skills
        DROP CONSTRAINT IF EXISTS operational_skills_active_version_id_fkey;
      ALTER TABLE operational_skills
        ADD CONSTRAINT operational_skills_active_version_id_fkey
        FOREIGN KEY (active_version_id) REFERENCES operational_skill_versions(id)
        ON DELETE SET NULL;

      CREATE TABLE IF NOT EXISTS operational_skill_conflicts (
        id SERIAL PRIMARY KEY,
        skill_id INTEGER NOT NULL REFERENCES operational_skills(id) ON DELETE CASCADE,
        skill_version_id INTEGER REFERENCES operational_skill_versions(id) ON DELETE SET NULL,
      ...
      CREATE TABLE IF NOT EXISTS operational_skill_actions (
        id SERIAL PRIMARY KEY,
        ...
        action_result TEXT NOT NULL
          CHECK (action_result IN ('success', 'blocked', 'pending_approval', 'error')),
        agent_id TEXT,
        metadata JSONB NOT NULL DEFAULT '{}',
        source_id TEXT NOT NULL DEFAULT 'default' REFERENCES sources(id) ON DELETE CASCADE,
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
      ...
      CREATE TABLE IF NOT EXISTS operational_skill_corrections (
        id SERIAL PRIMARY KEY,
        skill_id INTEGER NOT NULL REFERENCES operational_skills(id) ON DELETE CASCADE,
        skill_version INTEGER NOT NULL,
        ...
        evidence_slug TEXT,
        recompile_triggered BOOLEAN NOT NULL DEFAULT FALSE,
        corrected_by TEXT,
        source_id TEXT NOT NULL DEFAULT 'default' REFERENCES sources(id) ON DELETE CASCADE,
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
      );

      CREATE INDEX IF NOT EXISTS idx_operational_skills_source
        ON operational_skills(source_id);
      CREATE INDEX IF NOT EXISTS idx_operational_skill_versions_skill
        ON operational_skill_versions(skill_id, version DESC);
      CREATE INDEX IF NOT EXISTS idx_operational_skill_conflicts_open
        ON operational_skill_conflicts(source_id, status) WHERE status = 'open';
      CREATE INDEX IF NOT EXISTS idx_operational_skill_actions_slug
        ON operational_skill_actions(source_id, skill_slug, created_at DESC);
    `,
  },
];
