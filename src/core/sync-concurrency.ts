 ...
 * `modusbrain sync --all`. Sibling of `DEFAULT_PARALLEL_WORKERS`; the two
 ...
 * in modusbrain (e.g. `modusbrain eval prune --older-than`):
 *
...
/**
 * Resolve `MODUSBRAIN_MAX_CONNECTIONS` (a positive integer) or undefined when
...
export function resolveMaxConnections(): number | undefined {
  const raw = process.env.MODUSBRAIN_MAX_CONNECTIONS;
 ...
 * doctor nudge tells the operator to lower `MODUSBRAIN_POOL_SIZE`.
 *
 ...
 * inside a fan-out command (`modusbrain sync --all` + per-source workers)
 ...
 * is the lone legacy caller that bypasses it (`MODUSBRAIN_EMBED_CONCURRENCY
