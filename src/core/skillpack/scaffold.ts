/**
 * skillpack/scaffold.ts — `modusbrain skillpack scaffold <name>`.
 *
 * One-time, additive copy of a bundled skill into a host workspace. The
 * file-copy primitive is shared with harvest (host→modusbrain) via
 ...
 *      user owns it. To update, run `modusbrain skillpack reference <name>`
 *      and decide.
...
export function runScaffold(opts: ScaffoldOptions): ScaffoldResult {
  ...
  // symlink check or path confinement (the scaffold source is modusbrain's
  // own trusted bundle).
