...
import { brandConfigDir, brandEnv } from './branding.ts';
...
export function effectiveEnvDatabaseUrl(dir: string = process.cwd()): string | undefined {
  if (brandEnv('DATABASE_URL')) return brandEnv('DATABASE_URL');
...
export function loadConfig(): GBrainConfig | null {
  ...
  const merged = {
    ...
    ...(brandEnv('EMBEDDING_MODEL') ? { embedding_model: brandEnv('EMBEDDING_MODEL') } : {}),
    ...(brandEnv('EMBEDDING_DIMENSIONS') ? { embedding_dimensions: parseInt(brandEnv('EMBEDDING_DIMENSIONS')!, 10) } : {}),
    ...(brandEnv('EXPANSION_MODEL') ? { expansion_model: brandEnv('EXPANSION_MODEL') } : {}),
    ...(brandEnv('CHAT_MODEL') ? { chat_model: brandEnv('CHAT_MODEL') } : {}),
    ...(brandEnv('CHAT_FALLBACK_CHAIN')
      ? { chat_fallback_chain: brandEnv('CHAT_FALLBACK_CHAIN')!.split(',').map(s => s.trim()).filter(Boolean) }
      : {}),
    ...(brandEnv('EMBEDDING_MULTIMODAL')
      ? { embedding_multimodal: brandEnv('EMBEDDING_MULTIMODAL') === 'true' }
      : {}),
    ...(brandEnv('EMBEDDING_IMAGE_OCR')
      ? { embedding_image_ocr: brandEnv('EMBEDDING_IMAGE_OCR') === 'true' }
      : {}),
    ...(brandEnv('EMBEDDING_MULTIMODAL_MODEL')
      ? { embedding_multimodal_model: brandEnv('EMBEDDING_MULTIMODAL_MODEL') }
      : {}),
    ...(brandEnv('EMBEDDING_IMAGE_OCR_MODEL')
      ? { embedding_image_ocr_model: brandEnv('EMBEDDING_IMAGE_OCR_MODEL') }
      : {}),
    ...(brandEnv('RETRIEVAL_REFLEX')
      ? { retrieval_reflex: !(brandEnv('RETRIEVAL_REFLEX') === 'false' || brandEnv('RETRIEVAL_REFLEX') === '0') }
      : {}),
    ...(brandEnv('RETRIEVAL_REFLEX_WINDOW_TURNS') &&
      Number.isFinite(Number(brandEnv('RETRIEVAL_REFLEX_WINDOW_TURNS')))
      ? { retrieval_reflex_window_turns: Number(brandEnv('RETRIEVAL_REFLEX_WINDOW_TURNS')) }
      : {}),
    ...(brandEnv('REMOTE_CLIENT_SECRET') && fileConfig?.remote_mcp
      ? { remote_mcp: { ...fileConfig.remote_mcp, oauth_client_secret: brandEnv('REMOTE_CLIENT_SECRET') } }
      : {}),
  };
...
export function getDbUrlSource(): DbUrlSource {
  if (brandEnv('DATABASE_URL')) return 'env:MODUSBRAIN_DATABASE_URL';
