...
import { BRAND, brandEnv } from './branding.ts';

const DOTFILES = [BRAND.sourceDotfile, BRAND.legacySourceDotfile] as const;
...
export async function resolveSourceId(
  ...
  // 2. Env var.
  const env = brandEnv('SOURCE');
  if (env && env.length > 0) {
    if (!SOURCE_ID_RE.test(env)) {
      throw new Error(`Invalid MODUSBRAIN_SOURCE value "${env}". Must match [a-z0-9-]{1,32}.`);
    }
...
export async function resolveSourceWithTier(
  ...
  if (dotfile) {
    await assertSourceExists(engine, dotfile);
    return { source_id: dotfile, tier: 'dotfile', detail: BRAND.sourceDotfile };
  }
