/**
 * MODUSBRAIN_PLUGIN_PATH loader for host-repo subagent definitions (v0.15).
 *
 * Your OpenClaw (and future downstream agents) ship custom subagent defs
 * from their own repos. modusbrain discovers them at worker startup via
 * MODUSBRAIN_PLUGIN_PATH = colon-separated absolute paths (like $PATH). Each
 * path must contain a modusbrain.plugin.json manifest describing the plugin
 ...
 * version of modusbrain understands.
 */
...
export const SUPPORTED_PLUGIN_VERSION = 'modusbrain-plugin-v1';
...
/** Public entry point: load every plugin directory from MODUSBRAIN_PLUGIN_PATH. */
...
export function loadSinglePlugin(
  ...
  const manifestPath = path.join(rootDir, 'modusbrain.plugin.json');
  if (!fs.existsSync(manifestPath)) {
    return { error: 'missing modusbrain.plugin.json' };
  }
  ...
  if (manifest.plugin_version !== SUPPORTED_PLUGIN_VERSION) {
    return {
      error: `unsupported plugin_version "${manifest.plugin_version}" (modusbrain supports "${SUPPORTED_PLUGIN_VERSION}")`,
    };
  }
