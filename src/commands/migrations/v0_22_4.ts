 ...
 * a doctor subcheck (frontmatter_integrity), a top-level `modusbrain frontmatter`
 ...
 * writes a JSON report to ~/.modusbrain/migrations/v0.22.4-audit.json, and emits
 * one entry per source-with-issues to ~/.modusbrain/migrations/pending-host-work.jsonl.
 * It NEVER mutates brain content. The agent reads skills/migrations/v0.22.4.md
 * after upgrade and runs `modusbrain frontmatter validate <source-path> --fix` with
...
function modusbrainDir(): string {
  return join(process.env.HOME || '', '.modusbrain');
}
  ...
  try {
    ...
    for (const src of sourcesWithIssues) {
      ...
      const entry: PendingHostWorkEntry = {
        ...
        command: `modusbrain frontmatter validate ${src.source_path} --fix`,
      };
...
export const v0_22_4: Migration = {
  ...
  featurePitch: {
    ...
    description:
      'modusbrain v0.22.4 adds end-to-end frontmatter validation: a `modusbrain frontmatter` CLI ' +
      ...
      'report to ~/.modusbrain/migrations/v0.22.4-audit.json, and queues a TODO with the exact fix ' +
      'command for malformed frontmatter only. Missing frontmatter is treated as optional metadata ' +
      'coverage for broad document sources. Run `modusbrain frontmatter validate <source-path> --fix` ' +
      'to repair (creates centralized backups under ~/.modusbrain/backups/frontmatter). Ships frontmatter-guard.',
  },
  orchestrator,
};
