...
//
// v0.41.18.0 (A1, T13). CLI shell for `modusbrain onboard`. Thin wrapper over:
  ...
  if (auto && maxUsd === undefined) {
    ...
    if (maxUsdOff || tokenmax) {
      ...
      process.stderr.write(
        `modusbrain onboard --auto refuses without --max-usd N.\n` +
    ...
    },
    {
      onTargetUnreachable: (target, ceiling) => {
        process.stderr.write(
          ...
          `Configure missing prereqs (run modusbrain doctor --remediation-plan) or lower --target-score.\n`,
        );
      },
...
/**
 * v0.42 (T16): per-cluster narrative renderer for `modusbrain onboard --check --explain`.
 *
...
async function renderPackUpgradeExplain(
  ...
  try {
    ...
    process.stdout.write(
      ...
      `  modusbrain jobs submit unify-types --allow-protected --params '${JSON.stringify({ target_pack: targetPack })}'\n`,
    );
