/**
 * modusbrain eval export — stream captured eval_candidates rows as NDJSON (v0.21.0).
 *
 * Consumer: sibling modusbrain-evals repo, which imports the NDJSON as a
 ...
 *
 * The schema_version prefix lets modusbrain-evals detect format drift and
 * warn on unknown versions instead of silently misparsing.
 *
 * Usage:
 *   modusbrain eval export [--since 7d] [--limit N] [--tool query|search] > rows.ndjson
 */
...
function parseDurationToMs(s: string): number | null {
  // Accepts "30d", "7d", "1h", "90m", "3600s". Same shape as modusbrain eval prune + jobs prune.
...
function printHelp(): void {
  console.error(`modusbrain eval export — emit captured eval_candidates as NDJSON to stdout
...
OUTPUT:
  ...
  "schema_version": 1 so downstream consumers (modusbrain-evals) can
  detect format changes.

EXAMPLES:
  modusbrain eval export > rows.ndjson
  modusbrain eval export --since 7d --tool query | jq '.query'
  modusbrain eval export --limit 100 | head
`);
}
...
export async function runEvalExport(engine: BrainEngine, args: string[]): Promise<void> {
  ...
  for (const row of rows) {
    // Prefix every line with schema_version:1 so modusbrain-evals can detect
