/**
 * modusbrain mounts — manage connected modusbrains (v0.19.0, PR 0).
 *
 * A "mount" is a SEPARATE modusbrain DATABASE connected to your host agent.
 ...
 *   modusbrain mounts add <id> --path <path> --engine pglite|postgres [--db-url|--db-path]
 *   modusbrain mounts list [--json]
 *   modusbrain mounts remove <id>
 *
 * Not yet shipped (PR 1+):
 *   modusbrain mounts pin <id> <sha>        — freeze at a tested version (PR 1)
 *   modusbrain mounts sync [--id <id>]      — git pull + cache refresh (PR 1)
 *   modusbrain mounts enable/disable <id>   — toggle without removing (PR 1)
 *   modusbrain mounts add --mcp-url         — HTTP MCP transport + OAuth (PR 2)
 */
...
import { ModusBrainError } from '../core/types.ts';
import { modusbrainPath } from '../core/config.ts';

function getMountsDir(): string { return modusbrainPath(); }
// v0.40.3.0: MODUSBRAIN_MOUNTS_PATH override exists for tests (libuv caches
 ...
 *
 *  Unique tmp filename per call (pid + random). Two concurrent `modusbrain
 ...
 *  address, deferred to PR 1 under `modusbrain mounts sync --lock`). */
...
function parseAddArgs(args: string[]): AddArgs {
  if (args.length === 0) {
    throw new ModusBrainError(
      'Missing mount id',
      'modusbrain mounts add <id> --path <path> [flags]',
  ...
  for (let i = 1; i < args.length; i++) {
    ...
    const next = (flag: string): string => {
      const v = args[++i];
      if (!v) throw new ModusBrainError(`Missing value for ${flag}`, '', `Pass a value: ${flag} <value>`);
      return v;
    };
    if (a === '--path') path = next('--path');
      ...
      if (v !== 'postgres' && v !== 'pglite') {
        throw new ModusBrainError(`Invalid engine: "${v}"`, 'Must be "postgres" or "pglite"', 'Pass --engine pglite or --engine postgres');
      }
      engine = v;
    }
    ...
    else throw new ModusBrainError(`Unknown flag: ${a}`, '', 'See `modusbrain mounts add --help`');
  }

  if (!path) {
    throw new ModusBrainError('Missing --path', 'Every mount needs a local clone path (for skills + handlers)', 'Add --path /absolute/path/to/mount');
  }
  ...
  if (engine === 'postgres' && !database_url) {
    throw new ModusBrainError('postgres mount requires --db-url', '', 'Pass --db-url postgresql://...');
  }
  if (engine === 'pglite' && !database_path && !database_url) {
    throw new ModusBrainError('pglite mount requires --db-path', '', 'Pass --db-path /path/to/mount/.pglite');
  }
...
async function runAdd(args: string[]): Promise<void> {
  ...
  if (file.mounts.some(m => m.id === parsed.id)) {
    throw new ModusBrainError(
      ...
      `Use 'modusbrain mounts list' to see registered mounts`,
      `Remove the existing mount first: modusbrain mounts remove ${parsed.id}`,
    );
  }
  ...
  // Publish aggregated resolver + manifest to ~/.modusbrain/mounts-cache/. This
  ...
  // isn't inside a modusbrain repo, skip (a later mounts invocation from a
...
function runList(args: string[]): void {
  ...
  if (file.mounts.length === 0) {
    process.stdout.write(
      ...
      `Add a mount with:\n` +
      `  modusbrain mounts add <id> --path <path> --engine pglite --db-path <path>\n`,
    );
    return;
  }
...
function runRemove(args: string[]): void {
  if (args.length === 0) {
    throw new ModusBrainError(
      'Missing mount id',
      'modusbrain mounts remove <id>',
      `Run 'modusbrain mounts list' to see registered mounts`,
    );
  }
  const id = args[0];
  if (id === HOST_BRAIN_ID) {
    throw new ModusBrainError(
      ...
      `"host" is not a mount — it is the default brain from ~/.modusbrain/config.json`,
      `Use 'modusbrain init' to reconfigure the host brain`,
    );
  }
...
/**
 * Recompute + publish ~/.modusbrain/mounts-cache/{RESOLVER.md,manifest.json}.
 * Looks for the host skills dir via findRepoRoot(cwd). When not in a modusbrain
 ...
 * write already succeeded; a stale cache is recoverable via `modusbrain mounts
 * list` (PR 1 will add `modusbrain mounts sync --cache` for explicit refresh).
 */
function refreshMountsCache(): void {
  const repoRoot = findRepoRoot(process.cwd());
  if (!repoRoot) {
    process.stderr.write(
      'NOTE: mounts-cache not refreshed (not inside a modusbrain repo). ' +
      'Run `modusbrain mounts add|remove` from within a repo to publish ' +
...
function printHelp(): void {
  process.stdout.write(`modusbrain mounts — manage connected modusbrains (PR 0: direct transport only)
...
EXAMPLES
  # Mount a team-published yc-media modusbrain (PGLite)
  git clone https://github.com/yc-team/yc-media-modusbrain ~/modusbrains/yc-media
  modusbrain mounts add yc-media --path ~/modusbrains/yc-media --engine pglite \\
    --db-path ~/modusbrains/yc-media/.pglite

  # List registered mounts
  modusbrain mounts list

  # Remove a mount
  modusbrain mounts remove yc-media

v0.40.3.0 ADDITIONS
  modusbrain mounts enable <id>             — re-enable a disabled mount
  modusbrain mounts disable <id>            — toggle a mount off without removing
  modusbrain mounts trust-frontmatter <id>  — let this mount's per-page
                                          contextual_retrieval_mode
                                          frontmatter override the source
                                          default. Off by default for
                                          mounted brains; host is always
                                          trusted.
  modusbrain mounts untrust-frontmatter <id> — clear the trust flag.

NOT YET IMPLEMENTED (coming in PR 1/2)
  modusbrain mounts pin <id> <sha>          — freeze a mount at a tested version
  modusbrain mounts sync [--id <id>]        — git pull + refresh attestation
  modusbrain mounts add --mcp-url <url>     — HTTP MCP transport + OAuth
`);
}
