/**
 * v0.36.1.0 (T7) — `modusbrain calibration` CLI.
 *
 * Reads the latest calibration profile from the DB and prints it. Mirror of
 * the v0.29 `modusbrain salience` / `modusbrain anomalies` shape (pure data fn + JSON
 * formatter + human formatter + thin CLI dispatch).
 *
 * Sub-commands:
 *   modusbrain calibration                              — print active profile for default holder
 *   modusbrain calibration --holder <id>                — print for a specific holder
 *   modusbrain calibration --json                       — machine output
 *   modusbrain calibration --regenerate                 — run the calibration_profile phase now
 *   modusbrain calibration --undo-wave <version>        — D18 undo command (Lane D adds the impl)
 *   modusbrain calibration ab-report                    — D19 A/B harness report (Lane D adds the impl)
 *
...
export function formatProfileText(profile: CalibrationProfileRow | null, holder: string): string {
  if (!profile) {
    return (
      ...
      `  modusbrain dream --phase calibration_profile\n` +
...
function ctxFromCli(engine: BrainEngine, config: ModusBrainConfig, sourceId: string): OperationContext {
  return {
    engine,
    config,
