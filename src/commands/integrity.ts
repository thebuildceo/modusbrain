/**
 * modusbrain integrity — scan, report, and repair brain-integrity issues.
 *
 ...
 *
 * Subcommands:
 *   modusbrain integrity check              Read-only report to stdout
 *   modusbrain integrity auto               Three-bucket repair with confidence
 *   modusbrain integrity --dry-run          Same as auto, no writes
 *
 * Three-bucket confidence (contract with x_handle_to_tweet resolver):
 *   >= 0.8 → auto-repair through BrainWriter transaction
 *   0.5–0.8 → append to ~/.modusbrain/integrity-review.md for human review
 *   < 0.5 → skip, log to ~/.modusbrain/integrity.log.jsonl
 *
 * Progress is durable at ~/.modusbrain/integrity-progress.jsonl. Re-running
...
// Lazy: MODUSBRAIN_HOME may be set after module load.
const getReviewFile = () => modusbrainPath('integrity-review.md');
const getLogFile = () => modusbrainPath('integrity.log.jsonl');
const getProgressFile = () => modusbrainPath('integrity-progress.jsonl');
 ...
 * no resolver calls. Called by `modusbrain integrity check` for the full report
 * and by `modusbrain doctor` (non-fast) for a sampled health signal.
 *
...
export async function scanIntegrity(
  ...
  // postgres.js connection, so the gate keeps the MODUSBRAIN_DEBUG fallback
...
async function scanIntegrityBatch(
  ...
  // — modusbrain lint should reject stringly-typed validate at write time.
...
function cmdReview(): void {
  if (!existsSync(getReviewFile())) {
    console.log(`No review queue yet. Run: modusbrain integrity auto --confidence 0.8`);
    return;
  }
...
async function repairBareTweet(args: RepairArgs): Promise<void> {
  ...
  await writer.transaction(async (tx) => {
    ...
    await tx.appendTimeline(slug, {
      date: dateISO,
      source: 'modusbrain integrity --auto',
...
function printHelp(): void {
  console.log(`Usage: modusbrain integrity <subcommand> [options]
