/**
 * modusbrain report — Save a structured report to brain/reports/.
 *
 ...
 *   echo "report body" | modusbrain report --type meeting-sync --title "Meeting Sync"
 *   modusbrain report --type enrichment-sweep --dir /path/to/brain
 */
...
export async function runReport(args: string[]) {
  ...
  if (!reportType) {
    console.error('Usage: modusbrain report --type <name> --title "..." --content "..." [--dir <brain>]');
    console.error('  Or pipe content via stdin:');
    console.error('    echo "report body" | modusbrain report --type meeting-sync --title "Daily Sync"');
    console.error('');
