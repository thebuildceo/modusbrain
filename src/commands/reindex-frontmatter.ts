/**
 * v0.29.1 — `modusbrain reindex-frontmatter`.
 *
 ...
 *     column refreshed without a full `modusbrain sync`.
...
export interface ReindexFrontmatterOpts {
   ...
   * `modusbrain reindex --workers N` surfaces but currently INFORMATIONAL
...
export async function reindexFrontmatterCli(args: string[]): Promise<void> {
  ...
  if (!cfg) {
    console.error('No modusbrain config; run `modusbrain init` first.');
    process.exit(1);
  }
