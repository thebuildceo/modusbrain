/**
 * v0.32.7 CJK wave — `modusbrain reindex --markdown` sweep.
 *
 ...
 *
 * Driven by:
 *   - `modusbrain upgrade` post-upgrade hook (after the cost-estimate prompt).
 *   - Operators running `modusbrain reindex --markdown` directly.
 *
...
interface ReindexOpts {
   ...
   * embedding and the next `modusbrain embed --stale` pass fills them in.
...
export async function runReindex(engine: BrainEngine, args: string[]): Promise<ReindexResult> {
  ...
  if (!args.includes('--markdown')) {
    if (opts.json) {
      process.stdout.write(JSON.stringify({ error: 'modusbrain reindex requires a target flag, e.g. --markdown' }) + '\n');
    } else {
      process.stderr.write('Usage: modusbrain reindex --markdown [--limit N] [--dry-run] [--json] [--repo PATH]\n');
    }
    setCliExitVerdict(2);
