...
#
# Env overrides:
#   MODUSBRAIN_VERIFY_TIMEOUT       per-check wallclock cap, seconds (default 120)
#   MODUSBRAIN_VERIFY_LOG_DIR       where to write per-check logs (default tempdir)
#
# Exit codes:
#   0   all checks passed
...
TIMEOUT="${MODUSBRAIN_VERIFY_TIMEOUT:-120}"
...
if [ -n "${MODUSBRAIN_VERIFY_LOG_DIR:-}" ]; then
  LOG_DIR="$MODUSBRAIN_VERIFY_LOG_DIR"
  mkdir -p "$LOG_DIR" || { echo "ERROR: cannot create $LOG_DIR" >&2; exit 2; }
else
  LOG_DIR="$(mktemp -d /tmp/modusbrain-verify-XXXXXX)"
