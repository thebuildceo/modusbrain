 ...
 * (`modusbrain-cycle:<source_id>`) would by themselves let two PGLite cycles
 * for different sources run concurrently — which would corrupt the
 * single-writer invariant.
 *
 * Defense: cycle.ts acquires the GLOBAL file lock (`~/.modusbrain/cycle.lock`,
...
beforeAll(async () => {
  // MODUSBRAIN_HOME isolation so the file lock at ~/.modusbrain/cycle.lock doesn't
  // collide with the dev's real modusbrain. resetPgliteState would wipe the
  // config table so we don't use it here.
  modusbrainHome = mkdtempSync(join(tmpdir(), 'modusbrain-pglite-lock-'));
  process.env.MODUSBRAIN_HOME = modusbrainHome;
  process.env.MODUSBRAIN_HOME = modusbrainHome;
...
beforeEach(async () => {
  ...
  const lockPath = join(modusbrainHome, configDir, 'cycle.lock');
  if (existsSync(lockPath)) {
    try { unlinkSync(lockPath); } catch { /* best-effort */ }
  }
  brainDir = mkdtempSync(join(tmpdir(), 'modusbrain-cycle-pglite-ord-'));
});
...
describe('PGLite cycle: file lock + per-source DB lock ordering', () => {
  test('global file lock acquired during PGLite cycle (codex P0-C invariant)', async () => {
    ...
    await runCycle(engine, {
      ...
      yieldBetweenPhases: async () => {
        ...
        if (!lockFileExisted) {
          lockFileExisted = existsSync(join(modusbrainHome, configDir, 'cycle.lock'));
        }
      },
    });
    expect(lockFileExisted).toBe(true);
    // Lock file released on exit
    expect(existsSync(join(modusbrainHome, configDir, 'cycle.lock'))).toBe(false);
  });
  ...
  test('two PGLite cycles for DIFFERENT sources serialize (P0-D regression)', async () => {
    ...
    mkdirSync(join(modusbrainHome, configDir), { recursive: true });
    writeFileSync(
      join(modusbrainHome, configDir, 'cycle.lock'),
    ...
    await runCycle(engine, {
      ...
      yieldBetweenPhases: async () => {
        ...
        const rows = await engine.executeRaw<{ id: string }>(
          `SELECT id FROM modusbrain_cycle_locks WHERE id LIKE 'modusbrain-cycle%'`,
        );
        if (rows.length > 0) dbLockRowSeen = rows[0];
      },
    });
    expect(dbLockRowSeen).not.toBeNull();
    expect(dbLockRowSeen!.id).toBe('modusbrain-cycle:epsilon');
  });
  ...
  test('subsequent cycle after clean exit can acquire both locks', async () => {
    ...
    const dbRows = await engine.executeRaw<{ id: string }>(
      `SELECT id FROM modusbrain_cycle_locks WHERE id = 'modusbrain-cycle:zeta'`,
    );
