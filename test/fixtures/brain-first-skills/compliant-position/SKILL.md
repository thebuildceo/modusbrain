...
description: Skill that runs modusbrain search before external lookup
triggers:
  - "look up a topic"
---
...
reference (modusbrain search) appears strictly before the first external
reference (web_search), so the analyzer accepts it without requiring
the canonical callout.

## Workflow

1. Run `modusbrain search "topic"` to find existing brain pages.
