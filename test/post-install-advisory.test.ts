 ...
 * from the terminal output of `modusbrain init` and `modusbrain post-upgrade`.
 * These tests pin:
...
function scratchWorkspace(receiptSlugs: string[] | null): {
  ...
  if (receiptSlugs === null) {
    ...
    const block = [
      ...
      `<!-- modusbrain:skillpack:manifest cumulative-slugs="${slugList}" version="0.25.1" -->`,
      ...receiptSlugs.map(
...
describe('buildAdvisory — partial-install path', () => {
  it('lists ONLY missing skills when most are already installed', () => {
    ...
    expect(advisory).toContain('modusbrain skillpack scaffold book-mirror');
  });
  ...
  it('uses --all command when ALL recommended are missing (fresh workspace)', () => {
    ...
    expect(advisory).not.toBeNull();
    expect(advisory).toContain('modusbrain skillpack scaffold --all');
    expect(advisory).toContain('book-mirror');
