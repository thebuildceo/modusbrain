 ...
 * Use on Windows and Unix so migration orchestrators find `modusbrain` on PATH.
 */
...
/** Legacy config dir when only MODUSBRAIN_HOME is set. */
...
/** Env when tests intentionally exercise legacy ~/.modusbrain layout only. */
...
/** Shell script body for a `modusbrain`/`modusbrain` shim (Git Bash / Unix). */
...
/**
 * Migration orchestrators shell out to `modusbrain …`; Windows needs `.cmd` stubs too.
 */
export function makeCliShim(repoRoot: string, prefix = 'modusbrain-shim-'): { binDir: string; cleanup: () => void } {
  const binDir = mkdtempSync(join(tmpdir(), prefix));

  for (const name of ['modusbrain', 'modusbrain'] as const) {
