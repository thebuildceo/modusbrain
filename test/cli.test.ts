...
describe('CLI structure', () => {
  ...
  // rejected `modusbrain reindex` with "Unknown command: reindex" before the
...
describe('CLI dispatch integration', () => {
  ...
  test('sync --help prints sync-specific usage block without running sync (v0.37 D.4)', async () => {
    ...
    // CLI_ONLY_SELF_HELP so `modusbrain sync --help` reaches runSync's own
    ...
    const home = mkdtempSync(join(tmpdir(), 'modusbrain-cli-help-'));
    try {
      ...
      expect(existsSync(join(home, '.modusbrain', 'config.json'))).toBe(false);
      expect(exitCode).toBe(0);
    } finally {
