...
function makeTempDir(): string {
  return mkdtempSync(join(tmpdir(), 'modusbrain-cme-cli-'));
}
...
describe('cross-modal-eval CLI helpers', () => {
  ...
  test('MODUSBRAIN_HOME isolates receipts under <home>/.modusbrain/eval-receipts', async () => {
    const dir = makeTempDir();
    try {
      await withEnv({ MODUSBRAIN_HOME: dir }, async () => {
        const { modusbrainPath } = await import('../src/core/config.ts');
        const path = modusbrainPath('eval-receipts');
        expect(path.startsWith(dir)).toBe(true);
        expect(path.endsWith('eval-receipts')).toBe(true);
        expect(path.includes('.modusbrain')).toBe(true);
      });
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
