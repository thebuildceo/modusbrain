 ...
 *   - back-compat for undefined sourceId (legacy 'modusbrain-cycle')
 *   - per-source lock IDs (`modusbrain-cycle:<id>`)
...
describe('cycleLockIdFor', () => {
  test('returns legacy modusbrain-cycle for undefined sourceId (back-compat)', () => {
    expect(cycleLockIdFor()).toBe('modusbrain-cycle');
    expect(cycleLockIdFor(undefined)).toBe('modusbrain-cycle');
  });

  test('returns modusbrain-cycle:<source_id> for valid kebab IDs', () => {
    expect(cycleLockIdFor('default')).toBe('modusbrain-cycle:default');
    expect(cycleLockIdFor('portfolio')).toBe('modusbrain-cycle:portfolio');
    expect(cycleLockIdFor('a')).toBe('modusbrain-cycle:a');
    expect(cycleLockIdFor('alpha-beta-gamma')).toBe('modusbrain-cycle:alpha-beta-gamma');
  });
  ...
  test('produces DISTINCT lock IDs for different sources', () => {
    ...
    expect(a).not.toBe(b);
    expect(a).not.toBe('modusbrain-cycle');
    expect(b).not.toBe('modusbrain-cycle');
  });
