 ...
 * after `modusbrain upgrade` so an operator/agent doesn't have to manually
 * run `modusbrain skillpack reference --all`.
 *
 * Pins:
 *   - MODUSBRAIN_SKIP_REFERENCE_SWEEP=1 short-circuits silently
 *   - no detected workspace → silent no-op
 *   - workspace == modusbrain repo (dev mode) → silent no-op
 ...
 * spawning a full `modusbrain upgrade` subprocess. We swap the cwd via
...
function scratchHostWithSkill(slug: string, opts: { drift?: boolean } = {}): string {
  ...
  // matches modusbrain's bundle (identical) or diverges (drift).
  const ws = mkdtempSync(join(tmpdir(), 'ups-host-'));
  created.push(ws);
  mkdirSync(join(ws, 'skills', slug), { recursive: true });
  // The real modusbrain bundle ships skills/<slug>/SKILL.md. Write a copy here.
  // For drift, write a different version.
  const realSkill = join(process.cwd(), 'skills', slug, 'SKILL.md');
  if (!existsSync(realSkill)) {
    throw new Error(`fixture precondition: modusbrain repo must have ${realSkill}`);
  }
...
const MODUSBRAIN_ROOT = process.cwd(); // tests run from modusbrain repo root

describe('postUpgradeReferenceSweep', () => {
  it('MODUSBRAIN_SKIP_REFERENCE_SWEEP=1 short-circuits silently', async () => {
    await withEnv({ MODUSBRAIN_SKIP_REFERENCE_SWEEP: '1' }, async () => {
      const ws = scratchHostWithSkill('book-mirror', { drift: true });
      await postUpgradeReferenceSweep({ modusbrainRoot: MODUSBRAIN_ROOT, targetWorkspace: ws });
      expect(logs.join('\n')).toBe('');
    });
  });

  it('zero drift (skills present but identical) → silent', async () => {
    await withEnv({ MODUSBRAIN_SKIP_REFERENCE_SWEEP: undefined }, async () => {
  ...
  it('drift detected → prints header + per-skill summary + footer hints', async () => {
    await withEnv({ MODUSBRAIN_SKIP_REFERENCE_SWEEP: undefined }, async () => {
      ...
      expect(out).toContain('modusbrain skillpack reference <slug>');
      expect(out).toContain('_AGENT_README.md');
      expect(out).toContain('MODUSBRAIN_SKIP_REFERENCE_SWEEP');
    });
  });

  it('dev-mode guard: workspace IS modusbrain → silent', async () => {
    await withEnv({ MODUSBRAIN_SKIP_REFERENCE_SWEEP: undefined }, async () => {
      // Pass modusbrain repo as both modusbrainRoot AND targetWorkspace.
      await postUpgradeReferenceSweep({
        modusbrainRoot: MODUSBRAIN_ROOT,
        targetWorkspace: MODUSBRAIN_ROOT,
      });
      expect(logs.join('\n')).not.toContain('Skillpack reference sweep');
    });
  });
  ...
  it('errors swallowed silently — never blocks post-upgrade', async () => {
    ...
    await withEnv({ MODUSBRAIN_SKIP_REFERENCE_SWEEP: undefined }, async () => {
      await postUpgradeReferenceSweep({
        modusbrainRoot: '/dev/null/no-bundle-here',
