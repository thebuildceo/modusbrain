// v0.38 Phase C: modusbrain schema CLI smoke tests.
//
...
// Default-isolated MODUSBRAIN_HOME for every modusbrain() call. Without this,
// tests that read `~/.modusbrain/config.json` inherit the developer's real
...
let DEFAULT_MODUSBRAIN_HOME: string;

beforeAll(() => {
  DEFAULT_MODUSBRAIN_HOME = mkdtempSync(join(tmpdir(), 'modusbrain-schema-cli-default-'));
});

afterAll(() => {
  rmSync(DEFAULT_MODUSBRAIN_HOME, { recursive: true, force: true });
});
...
function modusbrain(
  ...
  // any subprocess test that needs MODUSBRAIN_HOME isolation.
  const env = { ...process.env, MODUSBRAIN_HOME: DEFAULT_MODUSBRAIN_HOME, ...extraEnv };
...
describe('modusbrain schema CLI (Phase C)', () => {
  ...
  test('schema show gbrain-base prints manifest details', () => {
    const r = modusbrain(['schema', 'show', 'gbrain-base']);
    expect(r.code).toBe(0);
  ...
  test('schema validate gbrain-base passes', () => {
    const r = modusbrain(['schema', 'validate', 'gbrain-base']);
    expect(r.code).toBe(0);
  ...
  test('schema active reports default resolution', () => {
    const r = modusbrain(['schema', 'active']);
    expect(r.code).toBe(0);
  ...
  test('schema show unknown-pack errors with hint', () => {
    const r = modusbrain(['schema', 'show', 'nonexistent-pack']);
  ...
  test('unknown subcommand exits with hint', () => {
    const r = modusbrain(['schema', 'frobnicate']);
    expect(r.code).toBe(2);
    expect(r.stderr).toContain('Unknown schema subcommand');
  });

  test('schema use without arg shows usage hint', () => {
    const r = modusbrain(['schema', 'use']);
    expect(r.code).toBe(2);
    expect(r.stderr).toContain('Usage:');
  });
});

describe('modusbrain schema use (Phase C, gap-fill T3)', () => {
  let home: string;

  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), 'modusbrain-schema-use-'));
  });
  ...
  test('writes schema_pack to ~/.modusbrain/config.json on happy path', () => {
    const r = modusbrain(['schema', 'use', 'gbrain-base'], { MODUSBRAIN_HOME: home });
    expect(r.code).toBe(0);
    expect(r.stdout).toContain('Active schema pack set to: gbrain-base');
    expect(r.stdout).toContain('schema active');
    const cfgPath = join(home, '.modusbrain', 'config.json');
  ...
  test('unknown pack rejected with exit 1 + paste-ready hint', () => {
    const r = modusbrain(['schema', 'use', 'no-such-pack-xyz'], { MODUSBRAIN_HOME: home });
