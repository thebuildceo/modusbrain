/**
 * Tests for resolveModusbrainCliPath() — picks the right executable to supervise
 ...
 * caused the bug. The canonical resolution is the `modusbrain` shim on PATH.
 */

import { describe, test, expect } from 'bun:test';
import { resolveModusbrainCliPath } from '../src/commands/autopilot.ts';

describe('resolveModusbrainCliPath', () => {
  test('returns a non-empty string or throws with a clear install hint', () => {
    let path: string;
    try {
      path = resolveModusbrainCliPath();
    } catch (e) {
      // Machine without modusbrain on PATH and no compiled binary: throw is
  ...
  test('NEVER returns a path ending in .ts (regression guard — Bug 4)', () => {
    ...
    try {
      const path = resolveModusbrainCliPath();
  ...
  test('shim on PATH wins over argv[1]=cli.ts', () => {
    // If `which modusbrain` resolves (most dev machines), the resolver should
    ...
    try {
      const path = resolveModusbrainCliPath();
      // On a machine where `which modusbrain` resolves, path ends in /modusbrain.
  ...
  test('accepts argv[1]=/modusbrain when shim is absent (compiled binary)', () => {
    // If the machine has neither shim nor compiled exec, but argv[1]
    // happens to be a literal /modusbrain path (direct invocation), accept it.
    const origArg1 = process.argv[1];
    process.argv[1] = '/usr/local/bin/modusbrain';
    try {
      const path = resolveModusbrainCliPath();
      // On a machine with `which modusbrain`, we get the shim. On a machine
      // without, argv[1] fallback fires. Either way the result is valid.
      expect(path.endsWith('/modusbrain') || path.endsWith('\\modusbrain.exe')).toBe(true);
    } finally {
      process.argv[1] = origArg1;
    }
  });
