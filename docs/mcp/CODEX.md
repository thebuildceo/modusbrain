...
```bash
modusbrain auth create "codex"
modusbrain connect https://YOUR-DOMAIN.ngrok.app/mcp --token modusbrain_xxx --agent codex
```

This prints a copy-paste block. Or wire it up directly and smoke-test the token:

```bash
modusbrain connect https://YOUR-DOMAIN.ngrok.app/mcp --token modusbrain_xxx --agent codex --install
```
...
```bash
export MODUSBRAIN_REMOTE_TOKEN=modusbrain_xxx
codex mcp add modusbrain --url https://YOUR-DOMAIN.ngrok.app/mcp \
  --bearer-token-env-var MODUSBRAIN_REMOTE_TOKEN
```

Codex stores the env-var *name* (`MODUSBRAIN_REMOTE_TOKEN`), not the token itself, and
...
> ON for `modusbrain init` brains, OFF for brains upgraded from older releases). Enable
> it on the host: `modusbrain config set mcp.publish_skills true`. The core tools
...
```bash
codex mcp remove modusbrain
```

## Notes

- The token is a long-lived, full-access secret. Keep `MODUSBRAIN_REMOTE_TOKEN` out of
