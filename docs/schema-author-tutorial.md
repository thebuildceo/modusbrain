...
- modusbrain v0.40.7.0 or later (`modusbrain --version` to check)
- A brain that's been initialized (`modusbrain init` already run; either PGLite or Postgres is fine)
...
Output: `Forked 'gbrain-base' → 'mine' at ~/.modusbrain/schema-packs/mine/pack.json`.
...
- An audit row landed in `~/.modusbrain/audit/schema-mutations-YYYY-Www.jsonl` with your type name SHA-8-redacted and the prefix's first segment only (`people`) for privacy.

Verify the type is in the pack:

```bash
modusbrain schema explain researcher
```
...
```bash
modusbrain schema stats --json | jq '.aggregate, .dead_prefixes'
```
...
You now have:
- A fork of `gbrain-base` named `mine` at `~/.modusbrain/schema-packs/mine/pack.json`, active in your brain via `~/.modusbrain/config.json`.
...
- 3 pages typed as `researcher` (backfilled from disk via `modusbrain schema sync --apply`).
- A query path that routes through the new type: `modusbrain whoknows` reads the pack and includes `researcher` in its type filter.
...
- **Reference:** `modusbrain schema --help` for the full 22-verb CLI surface; CLAUDE.md's "Schema Cathedral v3 (v0.40.7.0)" section for the module-by-module architecture.
