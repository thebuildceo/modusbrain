...
description: Auto-detect and repair broken integrations.
icon: wrench
---
...
v0.12.2 adds detection in `modusbrain doctor` and a standalone `modusbrain repair-jsonb`
...
```
modusbrain sync --force
# or per-page
modusbrain import <slug> --force
```

v0.12.2 cannot recover content that was already lost if you no longer have
the source markdown file. `modusbrain doctor` tells you which pages look short;
