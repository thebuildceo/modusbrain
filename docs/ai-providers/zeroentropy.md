...
Both land in modusbrain v0.35.0.0 behind the openai-compatible recipe path,
...
```bash
export MODUSBRAIN_EMBEDDING_MODEL=zeroentropyai:zembed-1
export MODUSBRAIN_EMBEDDING_DIMENSIONS=2560
```
...
```bash
modusbrain embed --stale --limit 50    # smoke a small batch
...
```bash
modusbrain models doctor --json | jq '.probes[] | select(.touchpoint=="embedding_config")'
```
...
The reranker is the bigger story: modusbrain had no cross-encoder reranker
...
```bash
modusbrain models doctor --json | jq '.probes[] | select(.touchpoint=="reranker_config")'
```
...
`~/.modusbrain/audit/rerank-failures-YYYY-Www.jsonl` (ISO-week rotation).
...
| `reranker_health` doctor warns about auth | `ZEROENTROPY_API_KEY` not set or invalid | Re-export the env var; `modusbrain models doctor` to verify |
