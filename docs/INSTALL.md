...
modusbrain init --pglite                  # 2 seconds; no server
modusbrain skillpack scaffold --all       # 43 skills scaffolded into your agent workspace
modusbrain doctor                         # green checks all the way down
```
...
> **If `bun install -g` hits a postinstall error** (Bun blocks postinstall hooks in some environments), the CLI prints a recovery hint pointing at [#218](https://github.com/garrytan/gbrain/issues/218). Run `modusbrain doctor` to diagnose, then `modusbrain apply-migrations --yes` manually. The deterministic fallback is `git clone https://github.com/garrytan/gbrain.git ~/modusbrain && cd ~/modusbrain && bun install && bun link`.

The init flow detects your repo size and suggests Supabase for brains > 1000 markdown files. To switch later:

```bash
modusbrain migrate --to supabase     # PGLite → Postgres
modusbrain migrate --to pglite       # Postgres → PGLite (rare)
```

For shared / large / multi-machine deployments (a team or company brain with multiple users hitting one server over HTTP MCP with OAuth scoping per user), follow the dedicated walkthrough: **[Tutorial: set up ModusBrain as your company brain](tutorials/company-brain.md)**.

API keys live in `~/.modusbrain/config.json` (file plane) or env vars (`OPENAI_API_KEY`, `ZEROENTROPY_API_KEY`, `VOYAGE_API_KEY`, `ANTHROPIC_API_KEY`). Set via CLI:
...
```bash
modusbrain import ~/my-knowledge      # bulk-import a markdown folder
modusbrain sync --watch               # live-sync a git repo (autopilot mode)
modusbrain autopilot --install        # background daemon for nightly enrichment
```

**Wire this same local brain into your coding agent** — zero server, zero token:

```bash
claude mcp add modusbrain -- modusbrain serve    # Claude Code
codex  mcp add modusbrain -- modusbrain serve    # Codex
```

The agent spawns `modusbrain serve` as a stdio subprocess against your local brain. Full walkthrough (both this local path and connecting to a remote brain), plus the brain-first protocol to paste into `CLAUDE.md` / `AGENTS.md`: **[Give your coding agent a memory](tutorials/connect-coding-agent.md)**.

## 3. MCP server (any MCP client)

```bash
modusbrain serve                      # stdio MCP (Claude Desktop / Code / Cursor)
modusbrain serve --http               # HTTP MCP with OAuth 2.1 + admin dashboard
```

**Wire a coding agent to a remote brain in one command** (when you have an HTTP
server + a bearer token): `modusbrain connect` prints a paste-ready setup block, or
`--install` runs it and smoke-tests the token.

```bash
modusbrain auth create "claude-code"
modusbrain connect https://your-host/mcp --token modusbrain_xxx                      # Claude Code (default)
modusbrain connect https://your-host/mcp --token modusbrain_xxx --agent codex        # Codex (env-var bearer)
modusbrain connect https://your-host/mcp --agent perplexity --oauth --register   # Perplexity (OAuth)
```
...
```bash
modusbrain init --mcp-only            # configures remote MCP, skips local DB
```
...
```bash
modusbrain doctor --json              # full health check
modusbrain models                     # which AI models are configured for what
modusbrain models doctor              # 1-token probe per configured model
```

If anything's yellow, `modusbrain doctor` names the fix command in the message. Most issues are missing API keys or stale schema (`modusbrain upgrade --force-schema`).
