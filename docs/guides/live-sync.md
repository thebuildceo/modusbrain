...
keeps serving the old text because nobody ran `modusbrain sync`. Stale search
...
`MODUSBRAIN_DIRECT_DATABASE_URL` to the **Session pooler** string (port 5432 on the
`pooler.supabase.com` host, IPv4), or enable Supabase's IPv4 add-on. Verify by
running `modusbrain sync` and checking that the page count in `modusbrain stats` matches
...
**OpenClaw:**
```
Name: modusbrain-auto-sync
Schedule: */15 * * * *
Prompt: "Run: modusbrain sync --repo /data/brain && modusbrain embed --stale
  Log the result. If sync errors mention an unreachable host or timeout,
  the direct connection isn't reachable over IPv4 (set
  MODUSBRAIN_DIRECT_DATABASE_URL to the Session pooler, or enable the IPv4 add-on)."
```

**Hermes:**
```
/cron add "*/15 * * * *" "Run modusbrain sync --repo /data/brain &&
  modusbrain embed --stale. Log the result." --name "modusbrain-auto-sync"
```
...
```bash
modusbrain sync --watch --repo /data/brain
```
...
1. **Always chain sync + embed.** Running `modusbrain sync` without
   `modusbrain embed --stale` leaves new chunks without embeddings. They exist
...
4. **A single un-parseable file can't wedge all indexing.** When a file fails
   ...
   way `MODUSBRAIN_SYNC_AUTOSKIP_AFTER` consecutive syncs (default 3, set `0` to
   disable) is auto-skipped so the rest of the brain keeps indexing past it.
   Skipped files don't disappear: `modusbrain doctor` keeps warning until you fix
   ...
   `modusbrain sync --skip-failed` to acknowledge a known-bad set yourself.
...
3. **Check embedded chunk count.** In `modusbrain stats`, the embedded chunk
   count should be close to the total chunk count. A large gap means
   `modusbrain embed --stale` isn't running after sync, leaving chunks invisible
   to vector search.
