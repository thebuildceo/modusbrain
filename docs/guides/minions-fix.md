...
set up (no `~/.modusbrain/preferences.json`, autopilot still inline, cron jobs
...
```
[FAIL] minions_migration: MINIONS HALF-INSTALLED (partial migration: 0.11.0). Run: modusbrain apply-migrations --yes
```

or

```
[FAIL] minions_config: MINIONS HALF-INSTALLED (schema v7+ but no ~/.modusbrain/preferences.json). Run: modusbrain apply-migrations --yes
```

For a machine-readable report (cron-friendly):

```bash
modusbrain skillpack-check --quiet && echo healthy || echo needs_action
modusbrain skillpack-check | jq -r '.actions[]'    # prints the exact commands to run
```
...
Reads `~/.modusbrain/migrations/completed.jsonl`, diffs against the TS
migration registry, runs whatever's pending. Seven phases:

```
A. Schema        modusbrain init --migrate-only
B. Smoke         modusbrain jobs smoke
C. Mode          prompt (or --yes default pain_triggered)
D. Prefs         write ~/.modusbrain/preferences.json
E. Host          AGENTS.md marker injection + cron rewrites for modusbrain
...
~29 non-modusbrain crons), the migration finishes with `status: "partial"`.
...
```bash
curl -fsSL https://raw.githubusercontent.com/thebuildceo/modusbrain/v0.11.1/scripts/fix-v0.11.0.sh | bash
```
...
5. Append `~/.modusbrain/migrations/completed.jsonl` with `status: "partial"`
...
Once v0.11.1 is installed, re-run `modusbrain apply-migrations --yes` to
...
cat ~/.modusbrain/preferences.json

# 2. Migration recorded
cat ~/.modusbrain/migrations/completed.jsonl
...
cat ~/.modusbrain/migrations/pending-host-work.jsonl 2>/dev/null || echo "(none — all host work is done)"

# 6. Doctor + skillpack-check should both be clean
modusbrain doctor
modusbrain skillpack-check --quiet && echo ok
```
...
- **Phase B smoke fails:** the schema didn't apply. Check
  `~/.modusbrain/config.json` has a valid `database_url` (or `database_path`
  for PGLite). Run `modusbrain init --migrate-only` directly and look at
  the error.
...
- **Pending host work never clears:** your host agent hasn't shipped
  ...
  `~/.modusbrain/migrations/pending-host-work.jsonl`, open
