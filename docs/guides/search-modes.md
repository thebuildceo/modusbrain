...
on user_asks_about(topic):
    ...
    if know_exact_slug(topic):
        ...
        result = modusbrain get <slug>
        # e.g., "Tell me about Pedro" -> modusbrain get pedro-franceschi
        ...
        results = modusbrain search "{name_or_keyword}"
        # e.g., "Find anything about Series A" -> modusbrain search "Series A"
        ...
        if chunk.confirms_relevance:
            full_page = modusbrain get <slug_from_chunk>

    elif topic.is_semantic_question:
        # MODE 2: Hybrid search -- semantic + keyword, needs embeddings
        results = modusbrain query "{natural language question}"
        # e.g., "Who do I know at fintech companies?" -> modusbrain query "fintech contacts"
...
# |---------|----------------------|------------------|---------|---------------------------------|
# | Keyword | modusbrain search "term" | No               | Fastest | Known names, exact matches      |
# | Hybrid  | modusbrain query "..."   | Yes              | Fast    | Semantic questions, fuzzy match  |
# | Direct  | modusbrain get <slug>    | No               | Instant | When you know the slug          |
