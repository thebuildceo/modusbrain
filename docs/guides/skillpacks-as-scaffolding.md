...
- `modusbrain skillpack uninstall` had its own data-loss safeguards (D8
...
modusbrain ships a new version, you ask "what changed?" — the agent reads
...
modusbrain skillpack scaffold book-mirror
...
```bash
modusbrain skillpack reference book-mirror
# These files live at <modusbrain-path> as reference. Read them and
...
```bash
cd ~/git/your-agent-repo
modusbrain skillpack migrate-fence
...
- Privacy linter scans the harvested files against
  `~/.modusbrain/harvest-private-patterns.txt` plus built-in defaults
...
- **New host repo, want a modusbrain skill** → `scaffold`
- **modusbrain shipped a new version, want to see what's changed**
...
- **Lift your fork's skill back into modusbrain** → `harvest` + the
