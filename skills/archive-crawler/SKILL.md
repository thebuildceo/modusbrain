...
explicitly set in `modusbrain.yml`. This is a deliberate safety fence against
the agent over-scoping a scan and ingesting sensitive content (tax PDFs,
medical records, credentials).

```yaml
# modusbrain.yml — the allow-list is mandatory
archive-crawler:
  scan_paths:
    - ~/Documents/writing/
    - ~/Dropbox/Archive/
...
in modusbrain.yml. Add explicit paths the agent is permitted to scan, then re-run.
...
scan_paths: ["paths from modusbrain.yml"]
---
...
## Source
- **Type:** [local|dropbox|...]
- **Allow-listed paths:** [from modusbrain.yml]
- **Total files:** [N]
- **Total size:** [X GB]
