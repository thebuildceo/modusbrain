...
description: |
  ...
  orchestration. Replaces the older `modusbrain-jobs` routing intent. Use when:
  submitting modusbrain jobs, shell/background tasks, spawning subagents,
...
triggers:
  - "modusbrain jobs submit"
  - "submit a modusbrain job"
  - "submit a shell job"
  - "shell job"
...
- Deterministic shell jobs (`modusbrain jobs submit shell ...`)
- LLM subagent jobs (`modusbrain agent run ...`)
...
| Condition | Action |
|---|---|
| User asks for deterministic command/script run | Shell job (CLI: `modusbrain jobs submit shell ...`) |
| User asks to "run in minions" + explicit command/argv | Shell job (CLI, `--params` with `cmd` or `argv`) |
| User asks for research/reasoning/iterative agent | Subagent job (CLI: `modusbrain agent run`) |
...
| Parallel work (2+ streams) | `modusbrain agent run --fanout-manifest` or parent + child subagents |
...
- **`MODUSBRAIN_ALLOW_SHELL_JOBS=1` must be set on the worker environment.**
...
- **Security:** flipping `MODUSBRAIN_ALLOW_SHELL_JOBS=1` authorizes arbitrary
...
- **Execution mode — pick one:**
  - **Postgres + daemon:** `modusbrain jobs work` runs a persistent worker that
    claims and executes jobs from the queue.
  - **PGLite + --follow:** `modusbrain jobs submit ... --follow` runs inline.
...
- **MCP boundary:** shell-job submission is CLI-only. `submit_job name="shell"`
  ...
  agent observes.
- **Verify setup:** after configuration, run `modusbrain jobs stats` (CLI) to
...
Command string form:
```
modusbrain jobs submit shell --params '{"cmd":"echo hello","cwd":"/abs/path"}'
```

Argv form (no shell expansion):
```
modusbrain jobs submit shell --params '{"argv":["bash","-lc","echo hello"],"cwd":"/abs/path"}'
```

Inline execution on PGLite or any one-shot deployment:
```
modusbrain jobs submit shell --params '{"cmd":"echo hello","cwd":"/tmp"}' --follow
```

Queue/lifecycle flags exposed by `modusbrain jobs submit --help`: `--queue`,
...
timings) from `get_job`. Use `modusbrain jobs stats` (CLI) for worker/queue
health dashboard.
...
**User-facing entrypoint:** `modusbrain agent run <prompt>` is the canonical way
...
submission requires `{allowProtectedSubmit: true}`, which `modusbrain agent run`
supplies.

## Phase 1: Submit

```
modusbrain agent run "Research Acme Corp revenue" --tools "search,query"
```
...
```
modusbrain agent run --fanout-manifest companies.json
```
...
Queue/priority/retry tuning is not exposed by `modusbrain agent run`; submit the
raw `subagent` handler via `modusbrain jobs submit` (requires CLI trust) if you
need those knobs.
...
modusbrain jobs stats                  # CLI — queue health dashboard
modusbrain agent logs ID --follow      # CLI — streaming transcript + heartbeat
```
...
- Don't spawn > 5 concurrent agents without checking `modusbrain jobs stats` first
- For subagent work, don't use `sessions_spawn` with `runtime: "subagent"` when Minions is available (use `modusbrain agent run` instead)
- Don't poll `get_job` in a tight loop (use `get_job_progress` for lightweight checks)

## Tools Used

- Submit a background job — `submit_job` (MCP, non-protected names only; shell jobs are CLI-only, subagent jobs via `modusbrain agent run`)
...
- Queue stats — `modusbrain jobs stats` (CLI; no MCP equivalent)
