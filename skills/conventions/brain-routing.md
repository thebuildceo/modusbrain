...
- **Brain** = which DATABASE. `--brain`, `MODUSBRAIN_BRAIN_ID`, `.modusbrain-mount`.
- **Source** = which REPO INSIDE the database. `--source`, `MODUSBRAIN_SOURCE`,
  `.modusbrain-source`.
...
1. Run `modusbrain mounts list` if you haven't seen the user's mounts yet.
2. Trust the resolver. If the user is in `~/team-brains/media/`, their
   `.modusbrain-mount` pins brain=media-team. Don't override that silently.
...
Bare `modusbrain query "X"` routes to the default brain's default source. That
...
`modusbrain` resolves the active source via `resolveSourceId()` in
`src/core/source-resolver.ts`. Seven tiers, highest priority first:

| # | Tier | Signal |
|---|---|---|
| 1 | `flag` | Explicit `--source <id>` CLI flag (or `--source-id <id>` on `modusbrain extract` / `modusbrain import`) |
| 2 | `env` | `MODUSBRAIN_SOURCE` environment variable |
| 3 | `dotfile` | `.modusbrain-source` file in CWD or any ancestor directory |
...
| 5.5 | `sole_non_default` | When tiers 1–5 missed AND exactly one registered source has a `local_path` AND isn't `'default'`, auto-route to it. Fires a one-time stderr nudge per CLI invocation. Suppress with `MODUSBRAIN_NO_SOLE_NON_DEFAULT_NUDGE=1`. |
...
Pre-fix, `modusbrain sync` from `/tmp` against a brain registering only
...
`sources.default` via `modusbrain sources default <id>` has stated intent that
wins over the auto-route. Archived sources are excluded from the count.

**v0.37.7.0 tooling:**

- `modusbrain sources current [--json]` echoes the resolved source AND
  which tier won. Run this before any destructive op to verify what
  you're about to target.
- `modusbrain sources current --source X` shows what an explicit flag
  WOULD resolve to (validates X exists in the sources table).

CLI commands honoring this chain: `modusbrain sync`, `modusbrain import`,
`modusbrain search`, `modusbrain extract` (via `--source-id <id>` since
`--source` is the fs|db data-source axis), `modusbrain graph-query`
(via `--include-foreign` for cross-source traversal).

**Trust boundary (v0.34.1.0):** the resolver is CLI-layer only.
Operations.ts handlers do NOT read `.modusbrain-source` or
`MODUSBRAIN_SOURCE`. MCP/remote callers go through
...
- The user is working in a specific repo (the `.modusbrain-source` dotfile
...
2. Check `modusbrain mounts list` for relevant brain ids.
...
- Ignoring `.modusbrain-mount` / `.modusbrain-source` dotfiles. They're load-bearing
