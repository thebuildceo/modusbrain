...
Sub-agents and fresh sessions inherit modusbrain tools but not the knowledge of
when and how to use them. This file is that knowledge.

## Available ModusBrain Tools

Your tool inventory includes these (prefixed `modusbrain__` in OpenClaw):

| Tool | Use for |
|------|---------|
| `modusbrain__search` / `search` | Keyword search — fast, always works |
| `modusbrain__query` / `query` | Hybrid search (keyword + semantic) — best quality |
| `modusbrain__get_page` / `get_page` | Direct page read when you know the slug |
| `modusbrain__get_links` / `get_links` | Outgoing links from a page |
| `modusbrain__get_backlinks` / `get_backlinks` | Who references this entity |
| `modusbrain__get_timeline` / `get_timeline` | Dated events for an entity |
| `modusbrain__resolve_slugs` / `resolve_slugs` | Fuzzy slug resolution |
| `modusbrain__traverse_graph` / `traverse_graph` | Walk the relationship graph |
| `modusbrain__put_page` / `put_page` | Create or update a brain page |
| `modusbrain__add_timeline_entry` | Add a dated event |
| `modusbrain__add_link` | Add a relationship edge |

Tool names vary by transport (MCP uses short names, OpenClaw plugin uses
`modusbrain__` prefix). Both work. Use whichever your environment provides.
...
- **After any brain page write:** trigger a sync so new pages are searchable.
  In OpenClaw: `modusbrain__sync_brain`. From CLI: `modusbrain sync --no-pull`.
...
| `projects/` | project | `projects/modusbrain.md` |
...
out; the `modusbrain doctor --fix` auto-add of the canonical Convention callout
