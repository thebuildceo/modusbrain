...
version: 1.0.0
description: |
  Run `modusbrain skillpack-check` to produce an agent-readable JSON health report
  for the modusbrain install. Wraps `modusbrain doctor` + `modusbrain apply-migrations
...
Running `modusbrain skillpack-check` returns a JSON report with:
...
- **`doctor`**: full `modusbrain doctor --fast --json` output (filesystem checks).
...
- **Daily cron** (e.g. your OpenClaw's `morning-briefing`): `modusbrain skillpack-check --quiet`.
...
- **On demand**: `modusbrain skillpack-check` for the full JSON when debugging.
...
- `modusbrain apply-migrations --yes` — A migration is pending or half-finished.
...
- `modusbrain embed --stale` — Embeddings are stale.
- `modusbrain check-backlinks --fix` — Dead links or missing back-links.
...
Treat as urgent. Probably means the modusbrain binary is missing from `$PATH` or
a required subcommand crashed. Check:

1. `which modusbrain` returns a path
2. `modusbrain --version` exits 0
3. `~/.modusbrain/` is accessible
...
```json
{
  ...
  "healthy": false,
  "summary": "modusbrain skillpack needs attention: 1 action(s) — modusbrain apply-migrations --yes",
  "actions": ["modusbrain apply-migrations --yes"],
  "doctor": {
    "exit_code": 1,
    "checks": [
      { "name": "minions_migration", "status": "fail", "message": "MINIONS HALF-INSTALLED (partial migration: 0.11.0). Run: modusbrain apply-migrations --yes" }
    ]
  },
  "migrations": {
    "applied_count": 0,
...
- `modusbrain doctor` — the underlying filesystem + DB check. skillpack-check
  composes this.
- `modusbrain apply-migrations --list` — the migration status view.
