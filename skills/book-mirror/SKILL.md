...
book-mirror runs as a CLI command (`modusbrain book-mirror`), NOT as a pure
...
4. ANALYZE   → `modusbrain book-mirror` fans out N read-only subagents.
...
Resolve `$BRAIN_DIR` from the modusbrain config (`modusbrain config get sync.repo_path`)
...
1. **Templates: USER.md and SOUL.md** if the user maintains them
   (modusbrain ships templates at `templates/USER.md` and `templates/SOUL.md`;
...
3. **Topic-relevant brain searches** tuned to the book's themes:
   - `modusbrain query "marriage"`, `modusbrain query "couples therapy"` for a
     marriage book.
   - `modusbrain query "founders"`, `modusbrain query "fundraising"` for a
     business book.
   - `modusbrain query "shame"`, `modusbrain query "anger"` for a psychology book.
4. **Brain pages for relevant entities** — `modusbrain query "<name>"` for
...
CONTEXT="$WORK/context.md"
{
  ...
  # modusbrain query the book's key themes, embed top results
  # ...
  echo
  echo "## Themes & cruxes"
...
## 4. Analysis: invoke `modusbrain book-mirror`
...
```bash
modusbrain put_page  # already done by the CLI; nothing to add here
...
- [ ] `modusbrain book-mirror --chapters-dir … --context-file … --slug … --title …` returned exit 0.
