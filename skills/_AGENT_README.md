...
You (the agent) are running on a host that scaffolded modusbrain skills here. This
...
host, not by modusbrain. Don't treat them as modusbrain artifacts.
...
That pattern was retired in modusbrain v0.36. Routing lives in frontmatter now.
...
They are reference code that the modusbrain CLI calls. You do not run them
directly unless the SKILL.md tells you to.

## Updates — when modusbrain ships a new version

The user runs `modusbrain upgrade`. Skill files DO NOT change automatically.
modusbrain becomes a reference library you compare against.

On every cold start, or any time the user mentions an upgrade, run:

```bash
modusbrain skillpack reference --all
```
...
```bash
modusbrain skillpack reference <slug>
```

This prints a unified diff between modusbrain's bundle and the local file. Read
it, then decide per file:

- **Local edit was intentional.** Keep your version. modusbrain is reference, not
  law.
- **Local edit was accidental drift** (e.g. you wrote stale content into the
  skill body). Either patch by hand, or run
  `modusbrain skillpack reference <slug> --apply-clean-hunks` (read the WARNING
  about two-way merge below first).
- **Genuinely new modusbrain change in a section you don't care about.** Skip or
  apply per your judgment.

For `missing` files (modusbrain added a new bundled skill since you scaffolded),
run `modusbrain skillpack scaffold <new-slug>` to bring it in.

### `reference --apply-clean-hunks` — two-way merge warning

This command does a two-way diff against modusbrain's current bundle. It does
NOT have access to the version you originally scaffolded. Consequence: if
the user's local file differs from modusbrain in ANY section (including
intentional user edits), those sections WILL be aligned to modusbrain.

Always run plain `modusbrain skillpack reference <slug>` first to inspect.
Use `--apply-clean-hunks` only when you're confident the local edits were
accidental or you want to fully reset to modusbrain's current bundle.
...
`docs/guides/skillpacks-as-scaffolding.md` in the modusbrain repo. The skill
