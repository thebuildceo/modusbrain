...
- Raw sources are preserved for provenance via `modusbrain files upload-raw` with automatic size routing.
...
2. **For each entity mentioned:**
   - Read the entity's page from modusbrain to check if it exists
   - If exists: update compiled_truth (rewrite State section with new info, don't append)
   - If new: check notability gate, then store the page in modusbrain with the appropriate type and slug
3. **Append to timeline.** Add a timeline entry in modusbrain for each event, with date, summary, and source citation.
4. **Create cross-reference links.** Link entities in modusbrain for every entity pair mentioned together, using the appropriate relationship type.
...
2. **For each entity detected:**
   ...
   - **If yes:** load context with `modusbrain get <slug>`. Use the compiled truth to
   ...
   - **If no:** assess notability (see `skills/_brain-filing-rules.md`). If the entity
     is worth tracking, create a new page with `modusbrain put <type/slug>` and populate
     with what you know.
3. **After creating or updating pages:** sync to modusbrain:
   ```bash
...
**Use `modusbrain files upload-raw` for automatic size routing:**
```bash
...
- `modusbrain files signed-url <storage-path>` -- generate 1-hour signed URL for viewing/sharing
- `modusbrain files restore <dir>` -- download back to local from cloud storage

Use `put_raw_data` in modusbrain to store raw API responses and metadata (JSON, not binary).
...
- Store raw data in modusbrain (put_raw_data)
