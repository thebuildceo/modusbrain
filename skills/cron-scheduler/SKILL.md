...
When the brain has 2+ active sources (anything `modusbrain sources list` shows
...
```cron
*/5 * * * * modusbrain sync --all --parallel 4 --workers 4 --skip-failed
```
...
**Avoid (legacy)**: separate `modusbrain sync --source default` and
`modusbrain sync --source zion-brain` entries staggered by 5 minutes. They
require manual deconfliction every time a new source is added, and a
slow source can race a fast source on the legacy global `modusbrain-sync`
lock (v0.40.3.0+ uses per-source `modusbrain-sync:<sourceId>` locks but the
per-source cron pattern doesn't benefit from the parallelism that
`--all --parallel` actually delivers).

`modusbrain doctor` surfaces the recommended line as a `sync_consolidation`
...
- Separate per-source `modusbrain sync --source <id>` cron entries when
  `modusbrain sync --all --parallel N --workers N` would replace them with
