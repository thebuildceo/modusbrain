...
description: |
  ...
  modusbrain's bundle so other clients can scaffold it. Editorial workflow:
...
triggers:
  - "harvest this skill"
  - "harvest my skill"
  - "publish this skill to modusbrain"
  - "lift this skill"
  - "share this skill"
  - "promote this skill"
  - "promote my skill"
  - "skill upstream"
  - "into the modusbrain core"
  - "modusbrain bundle"
mutating: true
writes_pages: false
writes_to:
...
# skillpack-harvest — Editorial workflow for lifting host skills into modusbrain

> **Convention:** see [_brain-filing-rules.md](../_brain-filing-rules.md) for
> file placement rules. This skill writes into modusbrain's own tree, not the
> brain repo's notes.

This skill is the inverse of `modusbrain skillpack scaffold`. Scaffold ships
skills downstream (modusbrain → host). Harvest lifts proven patterns
upstream (host → modusbrain) so they become references every other client
can scaffold.
...
3. `modusbrain skillpack harvest --dry-run` previewed the file set.
4. The real `modusbrain skillpack harvest <slug> --from <host>` succeeded
   with `status: harvested` (no privacy-lint hits).
5. `bun test test/skills-conformance.test.ts` passes on the new
   `skills/<slug>/SKILL.md`.
6. The user has reviewed the diff in modusbrain and explicitly approved
   the commit.

If any of these is incomplete, the skill is NOT yet harvested — the
files may sit in modusbrain's working tree, but they're not landed.

## Output Format

This skill produces three artifacts in modusbrain's working tree:
...
- **Skipping the dry-run.** Always preview first. Files land in
  modusbrain's working tree; cleanup is a `git checkout` away, but you
  shouldn't need to.
...
- **Harvesting a skill that's still in flux.** Wait until the host
  ...
  then re-harvest, and that churns modusbrain's bundle for no benefit.
...
- The user developed a skill in their host fork (Wintermute, Neuromancer,
  Zion, etc.) and wants other modusbrain clients to be able to use it
...
3. **The user owns the modusbrain checkout.** The harvest writes into
   modusbrain's working tree. They'll review and commit. Don't harvest
...
- What slug should the harvested skill have? (Slugs must be kebab-case,
  globally unique in the modusbrain bundle.)
...
```bash
modusbrain skillpack harvest <slug> --from <host-repo-root> --dry-run
```

The output shows:
- Which files would land in modusbrain's tree
...
3. **Fork-specific conventions → references**
   - Mentions of `<host-repo>/docs/...` files → either lift the doc
     into modusbrain OR replace with a generic placeholder explanation
...
```bash
modusbrain skillpack harvest <slug> --from <host-repo-root>
```

Default behavior:
- Path-confinement + symlink rejection at file copy
- Privacy linter runs against `~/.modusbrain/harvest-private-patterns.txt`
...
Outcomes:
- `harvested` — success, manifest updated, files in modusbrain's tree
- `lint_failed` — privacy linter caught something. Go back to Phase 3,
  scrub the host file, retry.
- `slug_collision` — modusbrain already has a skill at that slug. Either
  use a different slug, or pass `--overwrite-local` if you really
  mean to replace.

### Phase 5 — Verify in modusbrain
...
2. `modusbrain skillpack check --strict` — confirms no drift between
   bundle and modusbrain's own checkout.
3. `modusbrain skillpack list` — confirms the slug shows up in the bundle.
4. Review the diff: `cd <modusbrainRoot> && git diff -- skills/<slug>/`
5. Commit the additions in modusbrain (do NOT commit any leftover files
   in the host repo — harvest is a copy, not a move).

### Phase 6 — Downstream announcement (optional)

If other modusbrain clients should pick up the new skill:
...
```bash
modusbrain skillpack harvest <slug> --from <host-repo-root> --no-lint
```
...
- It does NOT publish to npm or a remote bundle. It writes to
  modusbrain's working tree; the user commits + ships via the normal
  modusbrain release process.
- It does NOT support `--all` (no batch harvest). One skill at a
  time keeps the editorial review tractable.

## Files this skill touches

- modusbrain's `skills/<slug>/` — every file in the host skill dir
  (copy)
- modusbrain's mirror path for declared paired sources
  (e.g. `src/commands/<slug>.ts` if the host SKILL.md declares it
  in frontmatter)
- modusbrain's `openclaw.plugin.json` — adds the slug to `skills:`
