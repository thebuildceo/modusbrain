...
feature_pitch:
  ...
  one_liner: "modusbrain eval suspected-contradictions samples retrieval pairs and flags conflicts, with doctor + MCP + dream-cycle integration."
---

# v0.32.6 Migration: Contradiction Probe

The agent ran this version migration when the user upgraded modusbrain. After
upgrade, this file is read by the auto-update agent which performs the
following steps automatically.

## Mechanical (handled by `modusbrain apply-migrations`)
...
- `eval_contradictions_runs` — one row per `modusbrain eval
...
modusbrain eval suspected-contradictions --query "what is acme's MRR" --top-k 5 --json

# Run against a queries file:
modusbrain eval suspected-contradictions --queries-file ~/.modusbrain/queries.jsonl --top-k 5

# Inspect findings without re-running:
modusbrain eval suspected-contradictions review --severity high
...
After the first probe run, `modusbrain doctor` shows a `contradictions` check
...
```bash
modusbrain doctor --json | jq '.checks[] | select(.name == "contradictions")'
```
...
didn't apply — run `modusbrain apply-migrations --yes` and re-check.
