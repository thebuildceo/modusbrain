...
users** — re-run `modusbrain init --migrate-only` and re-run `modusbrain extract all` if
...
1. **`modusbrain extract` no longer hangs on large brains.** The N+1 dedup pre-load
...
the v0.12.0 upgrade attempt. Run `modusbrain init --migrate-only` once to confirm
your schema is at version 10, then run `modusbrain extract all --dir <brain>` if
...
Re-running `modusbrain init --migrate-only` is a no-op for v9 (uses
...
### Re-running `modusbrain extract` on a previously-stuck brain

This is the most common case. Run:

```bash
modusbrain extract all --dir <brain-dir>
# or for live brains with no local checkout:
modusbrain extract all --source db
```
...
output of `modusbrain init --migrate-only` and `modusbrain config get version`.
