...
- `modusbrain frontmatter` CLI (validate / audit / install-hook)
- `frontmatter_integrity` subcheck under `modusbrain doctor`
...
- Audit-only migration that scans every registered source, writes
  `~/.modusbrain/migrations/v0.22.4-audit.json`, and queues per-source TODO
  entries to `~/.modusbrain/migrations/pending-host-work.jsonl`
- 0 warnings on `modusbrain check-resolvable` (down from 7 on master)
...
- `audit` — `scanBrainSources()` over every registered source; writes
  `~/.modusbrain/migrations/v0.22.4-audit.json`
- `emit-todo` — appends one entry per source-with-issues to
  `~/.modusbrain/migrations/pending-host-work.jsonl`. Each entry contains the
  exact `modusbrain frontmatter validate <source-path> --fix` command.
...
```bash
cat ~/.modusbrain/migrations/v0.22.4-audit.json
```
...
```bash
modusbrain frontmatter validate <source_path> --fix
```

`--fix` writes a `.bak` backup for every modified file. SLUG_MISMATCH errors
are surfaced for manual review (not auto-fixed) — modusbrain derives slugs from
...
```bash
modusbrain doctor --json | jq '.checks[] | select(.name == "frontmatter_integrity")'
modusbrain frontmatter audit --json | jq '.total'
```
...
- contents of `~/.modusbrain/migrations/v0.22.4-audit.json`
