...
date: 2026-05-05
feature_pitch: |
  modusbrain remembers what you SAW, not just what you typed. Drop a screenshot,
...
`modusbrain upgrade` runs `modusbrain apply-migrations --yes` automatically, which
...
- **Off by default**: nothing changes at runtime until you flip
  `embedding_multimodal: true` (DB plane via `modusbrain config set` or env via
  `MODUSBRAIN_EMBEDDING_MULTIMODAL=true`).

## Verification

```bash
modusbrain apply-migrations --yes
modusbrain doctor                # schema_version should be 36+
```

If `modusbrain doctor` reports a `schema_version` warning, run apply-migrations
manually and check `~/.modusbrain/upgrade-errors.jsonl` for details.
...
modusbrain config set embedding_model voyage:voyage-3-large
modusbrain config set embedding_dimensions 1024

# Required: flip the multimodal gate.
modusbrain config set embedding_multimodal true

# Optional: enable OCR via gpt-4o-mini (~$0.0003 per image).
modusbrain config set embedding_image_ocr true
```

Drop image files (PNG, JPG, JPEG, GIF, WEBP, HEIC, AVIF) into your brain
repo and run `modusbrain sync`. The image lands as a `type: image` page with:
...
Then re-run `modusbrain apply-migrations --yes`. If your provider doesn't ship
...
- `modusbrain query --image <path>` flag for image-similarity search.
