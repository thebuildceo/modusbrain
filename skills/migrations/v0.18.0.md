...
feature_pitch:
  ...
  description: |
    ...
    modusbrain backend can now hold multiple knowledge repos (wiki, gstack,
    ...
    Per-directory default via `.modusbrain-source` dotfile walk-up +
    `MODUSBRAIN_SOURCE` env var. Matches how kubectl / terraform / git
    scope context. `cd ~/yc-media && modusbrain query "X"` just works.
...
**Audience: host agents reading this after `modusbrain apply-migrations`
...
`modusbrain upgrade` chains to `modusbrain apply-migrations --yes`, which
runs:
...
modusbrain sources rename <id> <new-display-name>
modusbrain sources default <id>
modusbrain sources attach <id>    # write .modusbrain-source in CWD
modusbrain sources detach         # remove .modusbrain-source
...
Running `modusbrain sources attach gstack` inside `~/.gstack/` writes a
`.modusbrain-source` file containing the single word `gstack`. Any
modusbrain command run from that directory (or any subdirectory) auto-
selects `gstack` as the default source. `modusbrain sources detach`
removes the dotfile.
...
`modusbrain sources rename`, existing citations stay valid.
...
- `modusbrain sources import-from-github <url>` — deferred to a patch
...
standard `modusbrain sync` flow, it continues to target the default
source and sees no behavioral change. To start using multi-source:

```
# Register a new source
modusbrain sources add gstack --path ~/.gstack --no-federated

# Pin that directory to it so no --source flag is needed
cd ~/.gstack
modusbrain sources attach gstack
