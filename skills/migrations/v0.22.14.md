...
feature_pitch:
  ...
  body: |
    Bare `modusbrain jobs work` now ships with the same health protection the
    ...
    If you're using `modusbrain jobs supervisor`, you're already protected — the
    supervisor handles spawn-on-crash itself. The fail-stop concern only
    applies to direct `modusbrain jobs work` invocations.
---
...
If you run `modusbrain jobs work` directly (NOT under `modusbrain jobs supervisor`),
...
| cron watchdog | Cron entry that re-spawns when `pgrep -f "modusbrain jobs work"` is empty |
...
restart policy OR switch to `modusbrain jobs supervisor` (which spawns its own
child + restarts on crash internally).

## What ships

- DB liveness probes inside `modusbrain jobs work` (60s interval, 3 strikes → exit)
...
- `MODUSBRAIN_SUPERVISED=1` env var (set by supervisor on its child) causes the
...
**`modusbrain jobs work` now defaults `--max-rss 2048` (MB).** Pre-v0.22.14, bare
...
- You run `modusbrain jobs work` directly under a process manager (systemd, Docker,
...
- You run `modusbrain jobs supervisor` (the supervisor already defaulted to
...
If the user runs OpenClaw, Hermes, or any agent fork that uses `modusbrain jobs
work` directly (NOT under `modusbrain jobs supervisor`), they need to know. Check:

```bash
# Look for bare worker invocations in cron, systemd, Docker, launchctl, etc.
grep -r "modusbrain jobs work" ~/.config/systemd/ /etc/systemd/ 2>/dev/null | head
crontab -l 2>/dev/null | grep "modusbrain jobs work" | head
launchctl list 2>/dev/null | grep modusbrain | head
ps -ef | grep "modusbrain jobs work" | grep -v grep | grep -v supervisor
```
...
> "v0.22.14 changes `modusbrain jobs work` to default `--max-rss 2048` (MB).
...
**systemd (~/.config/systemd/user/modusbrain-worker.service):**

```ini
ExecStart=/usr/local/bin/modusbrain jobs work --queue default --concurrency 3 --max-rss 4096
# Or to opt out: --max-rss 0
```

Then `systemctl --user daemon-reload && systemctl --user restart modusbrain-worker`.

**cron (`crontab -e`):**

```cron
@reboot /usr/local/bin/modusbrain jobs work --queue default --concurrency 3 --max-rss 4096
```

**Docker compose:**

```yaml
command: ["modusbrain", "jobs", "work", "--queue", "default", "--concurrency", "3", "--max-rss", "4096"]
```

**launchctl (~/Library/LaunchAgents/com.user.modusbrain-worker.plist):**

```xml
<key>ProgramArguments</key>
<array>
  <string>/usr/local/bin/modusbrain</string>
  <string>jobs</string>
  <string>work</string>
...
```bash
modusbrain jobs work --health-interval 0 --max-rss 0
```

### 6. Verify

```bash
modusbrain jobs stats              # queue should be flowing normally
modusbrain doctor --json | jq '.'  # no critical warnings
ps -o rss= -p $(pgrep -f "modusbrain jobs work") | awk '{print $1/1024 " MB"}'
```
...
60s` segment (because `MODUSBRAIN_SUPERVISED=1` skips the child's self-monitor).
