...
feature_pitch:
  ...
  description: |
    ...
    scale. Retry, backoff, DLQ, unified `modusbrain jobs list` visibility, all
...
hosts) reading this AFTER `modusbrain apply-migrations` has run. There is no
...
1. Wait for the next scheduled fire (or trigger manually: `modusbrain jobs
   submit shell --params '...' --follow`).
2. Check `modusbrain jobs list --status completed --name shell --limit 5`
   for the result.
3. `modusbrain jobs get <id>` shows `exit_code`, `stdout_tail`, `stderr_tail`,
   `duration_ms`.
...
`MODUSBRAIN_ALLOW_SHELL_JOBS=1` on the worker, jobs sit in `waiting`
...
- **Automatic crontab rewrites.** Deferred to a future `modusbrain
...
- **Orphaned-shell-job stats.** `modusbrain jobs stats --orphaned` would
...
1. The worker runs with `MODUSBRAIN_ALLOW_SHELL_JOBS=1` (Postgres) or every
...
4. `modusbrain jobs stats` shows shell jobs completing at expected rates
