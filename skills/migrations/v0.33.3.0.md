...
`modusbrain upgrade` runs `modusbrain apply-migrations` automatically. Most users
...
- **CLI source-scoping default flipped** (Codex finding #7): `modusbrain
...
unless you watch `modusbrain doctor`'s autopilot stats.
...
The previously available CLI commands (`modusbrain code-callers <symbol>`)
...
```bash
# 1. Confirm upgrade
modusbrain --version    # 0.33.3.0

# 2. Confirm migration v51 ran
modusbrain apply-migrations --status | grep edges_backfilled_at_v0_34

# 3. Confirm the new MCP ops are exposed
modusbrain --tools-json | jq '.[] | select(.name | startswith("code_")) | .name'
...
modusbrain code-callers parseMarkdown --json | jq '.count'

# 5. (Optional) inspect the resolver's per-source progress
modusbrain doctor --json | jq '.checks | to_entries[] | select(.key | contains("code"))'
```

## Rollback

If you need to roll back: `git checkout` an older binary and run
`modusbrain doctor`. The v51 column + indexes stay in your DB (no-op for
older binaries; idempotent re-add on upgrade). No data is destroyed.

## What v0.34 will add on top

Per the v0.34 design doc (`/Users/garrytan/.gstack/projects/garrytan-modusbrain/garrytan-garrytan-miami-design-20260510-194145.md`):
...
- `modusbrain wiki` zero-LLM aggregator CLI
