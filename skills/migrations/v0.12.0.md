...
merged on top of v0.11.1 Minions). The migration is **automatic** — `modusbrain
post-upgrade` calls `modusbrain apply-migrations --yes` which invokes the v0.12.0
...
- **`modusbrain extract --source db`** for batch backfill on existing brains.
- **`modusbrain graph-query <slug>`** for typed-edge relationship traversal with
  cycle prevention.
- **Backlink-boosted hybrid search**: well-connected entities rank higher
  (`score *= 1 + 0.05 * log(1 + n)`).
- **Graph health metrics in `modusbrain health`**: `link_coverage`,
...
### Phase F — Record
Appends to `~/.modusbrain/migrations/completed.jsonl` so future `modusbrain
...
```bash
modusbrain init --migrate-only        # Phase A
modusbrain extract links --source db  # Phase C
modusbrain extract timeline --source db  # Phase D
modusbrain stats                      # Phase E (look for link_count > 0)
modusbrain graph-query <some-person-slug> --depth 2  # smoke test
```
...
```bash
modusbrain graph-query people/<slug> --type attended --depth 2
modusbrain graph-query companies/<slug> --type works_at --direction in
```
...
```bash
modusbrain config set auto_link false
```

Re-enable with `modusbrain config set auto_link true`.
