...
`modusbrain upgrade` runs:
1. `bun install` (refreshes deps; no new external deps in v0.34)
2. `modusbrain post-upgrade` → `modusbrain apply-migrations` → migration v56
...
```bash
modusbrain edges-backfill --all-sources
```
...
```bash
modusbrain doctor --json | jq '.checks.code_intel_coverage'
modusbrain call code_blast --symbol performSync
modusbrain call code_flow --entry_point runCycle
```
...
- **`modusbrain wiki`** — depends on clusters.
- **`modusbrain blast` / `modusbrain flow` CLI thin-shims.** Use `modusbrain call
