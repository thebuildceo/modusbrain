...
Every `modusbrain put` (and MCP `put_page`) now extracts entity references from the
...
### Extended `modusbrain extract` + new `modusbrain graph-query`

- `modusbrain extract links --source db` — backfill structured links by walking pages from
  the engine (works on live brains with no local checkout). FS-source still works:
  `modusbrain extract links --source fs --dir <path>` walks markdown files (v0.10.1 behavior preserved).
  Includes typed link inference, within-page dedup, content-hash-based `--since` filter.
- `modusbrain extract timeline --source db` — backfill structured timeline entries from page
...
### Schema migrations (auto-applied on `modusbrain init`)
...
### Graph health metrics in `modusbrain health`
...
```bash
modusbrain extract links --source db
```
...
If you have a local markdown checkout you'd rather walk: `modusbrain extract links
...
```bash
modusbrain graph-query <any-person-slug> --depth 2
```
...
Use `modusbrain graph-query <slug> --type <link_type>` to filter.

### Step 7: Auto-link is on by default
No action needed. Every future `modusbrain put` automatically updates the graph.
To disable: `modusbrain config set auto_link false`.

### Step 8: Record completion
```bash
mkdir -p ~/.modusbrain/migrations
echo '{"version":"0.10.3","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","status":"complete"}' >> ~/.modusbrain/migrations/completed.jsonl
```
