...
version: 0.23.0
feature_pitch:
  headline: "modusbrain dream now actually dreams: conversation transcripts → reflections, originals, and 25-year patterns."
  description: |
...
**Audience: host agents reading this after `modusbrain apply-migrations` has
...
`modusbrain upgrade` chains to `modusbrain apply-migrations --yes`, which runs:
...
`modusbrain dream` (and `modusbrain autopilot`) now run an 8-phase cycle:
...
modusbrain dream --phase synthesize --dry-run --json
```
...
modusbrain config set dream.synthesize.min_chars 2000
...
modusbrain config set dream.synthesize.exclude_patterns '["medical","therapy"]'
...
modusbrain config set dream.synthesize.cooldown_hours 12

# Patterns lookback window in days (default 30).
modusbrain config set dream.patterns.lookback_days 30

# Minimum distinct reflections needed to name a pattern (default 3).
modusbrain config set dream.patterns.min_evidence 3
```
...
- **Auto git commit + push.** v1 writes markdown files to `brain_dir`
  ...
  or let `modusbrain autopilot` handle it. v1.1 will add explicit
...
modusbrain dream --help    # shows the 8-phase pipeline

# Dry-run against a single transcript (cheap Haiku call only):
modusbrain dream --phase synthesize --input /tmp/some-transcript.txt --dry-run --json
```

If any step fails, file an issue with `modusbrain doctor` output and the
