...
```bash
ls ~/.modusbrain/integrations/twilio-voice-brain/heartbeat.jsonl 2>/dev/null
```
...
- Is there a public URL configured in `~/.modusbrain/config.json`?
...
with `modusbrain integrations show twilio-voice-brain`." Skip to Step 6.
...
`modusbrain integrations show twilio-voice-brain`."
...
echo '{"version":"0.8.0","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","voice_setup":"RESULT"}' >> ~/.modusbrain/migrations/completed.jsonl
```

Tell the user: "v0.8.0 migration complete. Run `modusbrain integrations doctor` to
