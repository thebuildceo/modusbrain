...
Run `modusbrain sync` and check for "Skipped ... does not match path-derived slug"
...
`modusbrain embed --all` now runs 30x faster (sliding worker pool, 20 concurrent
...
```bash
modusbrain embed --all
```

With the new parallel worker pool, this should complete in under 10 minutes
for most brains. Set `MODUSBRAIN_EMBED_CONCURRENCY=10` if you hit API rate limits.

### Step 3: Verify search works

```bash
modusbrain search "test query" --limit 5
```
...
echo '{"version":"0.9.1","ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","status":"complete"}' >> ~/.modusbrain/migrations/completed.jsonl
```
