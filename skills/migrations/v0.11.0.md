...
hosts) reading this AFTER `modusbrain apply-migrations` has run its
...
## Check what modusbrain left you

```bash
cat ~/.modusbrain/migrations/pending-host-work.jsonl
```
...
Modusbrain rewrites cron entries whose handler name matches a modusbrain
builtin (`sync`, `embed`, `lint`, `import`, `extract`, `backlinks`,
`autopilot-cycle`). For host-specific handlers (e.g. `ea-inbox-sweep`,
`frameio-scan`, `x-dm-triage`, `calendar-sync` on your OpenClaw), modusbrain
...
Autopilot already spawns `modusbrain jobs work` as a child. Configure it to
...
One test that exercises the full pipeline: scheduler → `modusbrain jobs
...
Run `modusbrain check-resolvable`. It validates reachability (is the skill
...
cron entry from `agentTurn ea-inbox-sweep` to `modusbrain jobs submit ...`
...
Modusbrain injects a marker into each AGENTS.md pointing to
...
`modusbrain apply-migrations`, and let modusbrain re-detect + update
...
- `skills/conventions/cron-via-minions.md` — the rewrite pattern
  modusbrain applies for builtins + the one your host ships for custom
  handlers.
