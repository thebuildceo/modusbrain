...
the skillpack). Then in your `modusbrain query` calls:
...
4. Omit both for canonical / definitional / code / graph queries
   (`who is X`, `what is X`, etc.) — modusbrain's heuristic defaults
   to `off`.
...
```bash
# 1. Confirm upgrade
modusbrain --version    # 0.29.1
modusbrain doctor --json | jq '.checks[] | select(.name | startswith("schema_version"))'

# 2. Recompute emotional weights (one-time after upgrade)
modusbrain dream --phase recompute_emotional_weight

# 3. Verify health checks
modusbrain doctor --json | jq '.checks[] | select(.name | startswith("salience_health") or startswith("effective_date_health"))'

# 4. Try the new axes
modusbrain query "what's been going on with X" --explain --json | jq '._resolved'
# expected: { salience: "on", recency: "on", salience_source: "auto_heuristic", recency_source: "auto_heuristic" }

modusbrain query "who is X" --explain --json | jq '._resolved'
# expected: { salience: "off", recency: "off", ... }

# 5. Date filter
modusbrain query "acme" --since 7d --until 2024-06-30 --json
```
...
modusbrain reindex-frontmatter --yes --force
...
with the doctor output and contents of `~/.modusbrain/upgrade-errors.jsonl`
(if it exists).
