...
2. New CLI command `modusbrain extract-conversation-facts` for bulk fact extraction over long-form conversation pages.
...
`modusbrain upgrade` + `modusbrain apply-migrations --yes` handles the binary + schema. The cycle phase and doctor check require no agent action.
...
1. **Estimate cost first.** For each source the user wants to backfill:
   ```bash
   modusbrain sources audit <source-id> --json | jq '.facts_backfill_estimate'
   ```
...
2. **Run a dry-run** to preview segmentation:
   ```bash
   modusbrain extract-conversation-facts --dry-run --slug <slug-of-one-conversation-page>
   ```
...
3. **Run with explicit cost cap.** Default cap is $5. Recommend matching the audit estimate (rounded up):
   ```bash
   modusbrain extract-conversation-facts --background --max-cost-usd <cap>
   modusbrain jobs follow <printed-job-id>
   ```
...
4. **Verify with doctor:**
   ```bash
   modusbrain doctor --json | jq '.checks[] | select(.name == "conversation_facts_backlog")'
   ```
...
- Extracted facts use `source = 'cli:extract-conversation-facts'`. To remove them in bulk (rare), the only path today is raw SQL: `DELETE FROM facts WHERE source LIKE 'cli:extract-conversation-facts%'`. A `modusbrain forget --where` bulk flag is a v0.42+ TODO.
