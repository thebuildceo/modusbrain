...
This migration introduces the ModusBrain integration recipe system. Existing users
should be informed about the new capabilities.

## What's New

ModusBrain now distributes **integration recipes** — markdown files that teach your
...
Phone calls create brain pages. Powered by Twilio + OpenAI Realtime + ModusBrain MCP.
...
### New: `modusbrain integrations` Command

Discover, check status, and monitor your integrations:

```bash
modusbrain integrations              # dashboard: show all senses + reflexes
modusbrain integrations list --json  # list available recipes (agent-parseable)
modusbrain integrations show <id>    # show recipe details + setup instructions
modusbrain integrations status <id>  # check secrets + health
modusbrain integrations doctor       # run health checks for all integrations
modusbrain integrations stats        # signal statistics
```
...
`docs/MODUSBRAIN_SKILLPACK.md` still works (backward compatible).

## Agent Instructions

When presenting this update to the user, say something like:

"ModusBrain v0.7.0 adds integration recipes — your brain can now get new senses
automatically. The first one is Voice-to-Brain: phone calls create brain pages.
Want me to show you what's available? Run `modusbrain integrations` to see the
dashboard."

If the user is interested in voice-to-brain:
1. Run `modusbrain integrations show twilio-voice-brain` to see what's needed
