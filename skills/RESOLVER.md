...
| "what search mode", "is my cache hot", "tune my retrieval", "compare search modes", "clear search overrides" | `modusbrain search modes/stats/tune` directly. See `skills/conventions/search-modes.md` |
| "eval results", "search benchmark", "haters-immune methodology", "regression check on retrieval" | `modusbrain eval run-all` / `modusbrain eval compare`. See `docs/eval/SEARCH_MODE_METHODOLOGY.md` |
...
| "get more out of modusbrain", "is my brain set up right", "weekly brain checkup", "advise me on my brain", "modusbrain advisor" | `skills/modusbrain-advisor/SKILL.md` |
...
| "Is modusbrain healthy?", morning health check, skillpack-check | `skills/skillpack-check/SKILL.md` |
| "harvest this skill into modusbrain", "publish this skill to modusbrain", "lift this skill upstream", "share this skill with other modusbrain clients", "promote my skill to modusbrain" | `skills/skillpack-harvest/SKILL.md` |
...
| "Spawn agent", "background task", "parallel tasks", "steer agent", "pause/resume agent", "modusbrain jobs submit", "submit a modusbrain job", "submit a shell job", "shell job" | `skills/minion-orchestrator/SKILL.md` |
...
| Trigger | Skill |
|---------|-------|
| "Set up ModusBrain", first boot | `skills/setup/SKILL.md` |
...
| "Brain health", "what features am I missing", "brain score" | Run `modusbrain features --json` |
| "Set up autopilot", "run brain maintenance", "keep brain updated" | Run `modusbrain autopilot --install --repo ~/brain` |
| "Upgrade modusbrain", "update modusbrain", "modusbrain update available", `UPGRADE_AVAILABLE`, "is modusbrain up to date" | `skills/modusbrain-upgrade/SKILL.md` |
...
| "add a page type", "add a type to my schema", "schema author", "schema mutate", "schema pack add", "my brain has untyped pages", "propose new types from my corpus", "backfill page types", "evolve my schema", "researcher type", "make X an expert type" (dispatcher for: modusbrain schema active/list/show/validate/graph/lint/stats/explain/use/downgrade/reload/init/fork/edit/diff/add-type/remove-type/update-type/add-alias/remove-alias/add-prefix/remove-prefix/add-link-type/remove-link-type/set-extractable/set-expert-routing/detect/suggest/review-candidates/review-orphans/sync) | `skills/schema-author/SKILL.md` |
| "unify my types", "migrate to gbrain-base-v2", "94 types to 14", "apply canonical taxonomy", "clean up my page types", "pack upgrade", "shrink type proliferation", "consolidate page types", "retype pages to canonical" (dispatcher for: modusbrain onboard --check, modusbrain onboard --check --explain, modusbrain jobs submit unify-types, modusbrain pages restore) | `skills/schema-unify/SKILL.md` |
