...
> skill's Phase 3 below uses `modusbrain eval cross-modal` instead — three
...
receipt under `~/.modusbrain/.modusbrain/eval-receipts/<slug>-<sha8>.json` (the
...
Models resolve through the modusbrain AI gateway. Configure once with:
...
from `~/.modusbrain/config.json` plus `process.env`.
...
modusbrain skillify check skills/<slug>/scripts/<slug>.mjs --json | \
  jq '.[] | .items[] | select(.name | contains("Cross-modal"))'
ls ~/.modusbrain/.modusbrain/eval-receipts/              # receipt landed
modusbrain check-resolvable --json | jq .ok          # resolver clean
```
...
1. **The skill tree on disk.** `skills/<slug>/SKILL.md`, `scripts/<slug>.mjs`,
   ...
   `modusbrain skillify scaffold <name>` and refined by the human/agent into a
   real implementation.
2. **A cross-modal eval receipt** at
   `~/.modusbrain/.modusbrain/eval-receipts/<slug>-<sha8>.json`. The sha-8 binds the
   receipt to the current `SKILL.md` content. `modusbrain skillify check`
   surfaces the status (`found` / `stale` / `missing`) as informational.
3. **An audit verdict** from `modusbrain skillify check`: `properly skilled` |
...
JSON output (`modusbrain skillify check --json`) includes the same fields plus
